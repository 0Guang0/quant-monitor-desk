...
name: create-skill
description: >-
  Authors agent skills (SKILL.md structure, description, progressive disclosure).
  Use when creating or editing a skill, or when asked about skill layout or discovery paths.
---

# Creating Agent Skills

This skill guides you through creating effective Agent Skills for **OpenCode** and **shared runtimes** (Codex via `~/.agents/skills/`). Skills are markdown files that teach the agent how to perform specific tasks: reviewing PRs using team standards, generating commit messages in a preferred format, querying database schemas, or any specialized workflow.
...
2. **Target location**: Personal vs project — OpenCode (`~/.config/opencode/skills/`), shared (`~/.agents/skills/`), or project (`.opencode/skills/`, `.agents/skills/`). Cursor users may also use `~/.cursor/skills/` (see Storage Locations).
...
- "Where should this skill be stored?" with options like
  ["Personal OpenCode (~/.config/opencode/skills/)",
   "Personal shared (~/.agents/skills/)",
   "Project (.opencode/skills/ or .agents/skills/)"]
...
Path tables and full skill sample: [reference.md](reference.md)

### Storage Locations

Personal: `~/.config/opencode/skills/` (OpenCode) · `~/.agents/skills/` (Codex/shared). Project: `.opencode/skills/` · `.agents/skills/`. **Never** host-bundled dirs (`~/.cursor/skills-cursor/`, `node_modules/.../skills/`).

Full table and cross-host sync: [reference.md](reference.md#storage-locations)
...
On hosts that support `disable-model-invocation: true` (e.g. Cursor), default it so the skill loads only when named explicitly. Omit on hosts that ignore the field, or when the agent should auto-invoke from ambient context.
...
2. Storage location (personal vs project; OpenCode vs `~/.agents/skills/`)
3. Trigger scenarios
...
5. Confirm the skill is in the correct storage path (not a bundled host dir)
...
Full `code-review` sample (directory layout + SKILL.md): [reference.md](reference.md#complete-example)
