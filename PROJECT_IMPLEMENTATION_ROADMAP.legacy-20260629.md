...
| 当前执行顺序     | `docs/implementation_tasks/README.md`                                                                                 | Batch 3F-R **CLOSED**；Batch 3G **CLOSED** @ R3G-03；Batch 3H **R3H-01～04 CLOSED** @ 2026-06-28；**R3H-06 CLOSED** @ 2026-06-29；**当前下一入口：** Round4 PASS 收口波次（Batch3V → R3H-07/10 → R3H-08 → R3H-05-GATE）；PASS 波次 SSOT：§5.0.2–§5.0.5；**DataSync 五类 job / 写库覆盖 / 增量读库** 根目录索引：**§5.0.6**；`R3H_PASS_EXECUTION_PLAN.md`；用户裁决 @ 2026-06-28。 |
...
```text
条件 A：Batch 3F-R 全部完成并通过 batch close criteria  ← **当前状态（R3FR-07 CLOSED）**
...
> **当前执行：** **Round4 PASS 收口波次**（非仅 R3H-05 文档审计）— 详见 §5.0.2–§5.0.6 与 `R3H_PASS_EXECUTION_PLAN.md`。  
...
Wave 1  R3H-06 Schema — **CLOSED** @ 2026-06-29；原阻塞 Wave 3 已解除
...
### 5.0.6 DataSync 编排与写库契约（根目录索引 SSOT）

> **本节定位：** 回答「FullLoad / Incremental / Backfill / RevisionAudit、增量先读库、批量补历史、任务级幂等、写库是否覆盖」分别在**哪个批次闭合**、**代码现状**、**去哪张任务卡**。  
> **避免倒查：** 以前这些条目散落在 `data_sync_orchestrator.md`、`ROUND2_GAPS_AND_DEVIATIONS.md`、`014` 任务卡、Batch6 地图、R3H PASS；**以本节为根目录唯一索引**，细节仍以模块设计与 canonical 任务卡为准。  
> **与 §8「Sync orchestration」行关系：** §8 只写 Batch05 最终 R6 确认；本节写**分阶段主能力与缺口**。

#### 5.0.6.1 五类 Sync Job → 实现阶段

| Job 类型                          | 设计 SSOT                                                       | 规划闭合阶段（主能力）                                                                                                                                                                                  | 代码现状 @ 2026-06-29                                                                                                          | 工单 / 登记入口                                                                                        |
| --------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------ |
| **Incremental**（日常增量）       | `docs/modules/data_sync_orchestrator.md` §12.3、§13.4.2         | **Round2** runner 金路径；**自动读库算窗口** → `UNRESOLVED_ISSUES_REGISTRY` **R3-PARTIAL-1** → **Batch6**；**真网日常产品化** → **R3H-08**；**production smoke** → **Batch05** §7.5                     | `IncrementalJobRunner` 存在；fetch→validate→gate→write 可跑；**未**统一接「读 clean 表 `max(trade_date)` / cursor 再定拉取窗」 | `ROUND2_GAPS_AND_DEVIATIONS.md` · `014_implement_data_sync_orchestrator.md` · `R3H_PASS` **LIVE-PROD** |
| **Backfill**（分段补历史）        | 同上 §12.4、§13.4.3                                             | **Round2** 分片骨架；**回填与 reconcile 一致性** → Batch3F **B3F-BR**；**生产级 broad backfill + CLI** → **Batch6**（`D2-P1-3` 等）；**试点 promote cap 放宽 / 真网** → **R3H-08**；smoke → **Batch05** | `BackfillShardRunner`：分片 + ResourceGuard + **每片** validate + write；sandbox promote 仍受 **≤120 天** 等 cap               | `BATCH_3F` **B3F-BR** · `014` · `R3-PARTIAL-1`                                                         |
| **FullLoad**（首次全量建库）      | 同上 §12.2                                                      | **DEFERRED** `D2-P1-1` → Round3+ / **Batch6** job matrix；**不是** Batch01 全历史产品入口（Batch01 **禁止** full-history）                                                                              | `run_full_load` → `raise_deferred_job_type`（故意未实现）                                                                      | `docs/AUDIT_DEFERRED_REGISTRY.md` **D2-P1-1** · `ROUND2_GAPS` §1                                       |
| **RevisionAudit**（源头修订检测） | 同上 §12.5、§13.4.4                                             | **DEFERRED** `D2-P1-1` → **Batch6**（`run_revision_audit` pytest）；Batch3F **B3F-SH** 最小 runner                                                                                                      | 极简 runner 存在；**非**完整 `content_hash` 对比 + 触发局部 backfill 产品                                                      | `D2-P1-1` · `014` §未决登记                                                                            |
| **Reconcile**（多源冲突后重抓）   | `data_sync_orchestrator.md` + `data_validation_and_conflict.md` | **Round2** 部分；**完整 re-fetch/compare** → **Batch6** `D2-P2-2` / **R3-PARTIAL-3**                                                                                                                    | `run_reconcile` 存在；不自动完整重抓                                                                                           | `ROUND2_GAPS` §2.2 · `R3-PARTIAL-3`                                                                    |

**Round4 前硬门禁（与上表关系）：** Incremental/Backfill **runner + 写库两道闸** 须在 **R3H-06 + R3H-08** 路径上可演示；**不要求** FullLoad / 完整 RevisionAudit / 生产 CLI 在 Round4 前 CLOSED（属 Batch6 + Batch05 smoke）。

#### 5.0.6.2 配套能力 → 实现阶段

| 能力                                                      | 设计 SSOT                                            | 规划闭合阶段                                                                                                                                               | 代码现状                                                             | 工单 / 登记                                                                   |
| --------------------------------------------------------- | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| **增量先读库再决定拉哪段**（watermark / cursor）          | `data_sync_orchestrator.md` §13.4.2 步骤 1–2         | **根目录以前无独立 ID**；闭合归属 **Incremental 产品化**：**Batch6**（R3-PARTIAL-1 余量）+ **R3H-08** 真网日常                                             | 设计步骤清晰；**无**统一 orchestrator 自动查 clean 表算 `date_start` | 本节 + `R3-PARTIAL-1`                                                         |
| **批量补历史（生产级）**                                  | FullLoad + Backfill + Parquet 归档                   | **Batch6** broad backfill/CLI；**R3H-08** 真网进 Tier A；**Batch05** bounded smoke；**018A**（Batch 2.5）仅为 Layer1 微观桥接，**不是** broad full-history | 分片 backfill 可跑；**无**一键「全市场 N 年」产品入口                | `018A`（scope 限定）· Batch6 · `PRODUCTION_COMPLETION_VERTICAL_SLICE_PLAN.md` |
| **任务级幂等**（同 `idempotency_key` 重复跑返回已有结果） | `docs/ops/idempotency_retry_dlq_policy.md`           | **Batch6** ops / 同步编排深化（与 retry/DLQ 同包）                                                                                                         | 策略文档有；backend **未见** `idempotency_key` 存取                  | `idempotency_retry_dlq_policy.md` · Batch6                                    |
| **写库幂等**（同主键不叠行）                              | `docs/modules/write_manager.md` · **G6-IDEMPOTENCY** | **R3H-06** + Batch3V                                                                                                                                       | `upsert_by_pk`：**DELETE 同 PK + INSERT 整行**                       | **R3H-06 CLOSED** @ 2026-06-29                                                |
| **抓取对象**（原材料 vs 建模指标）                        | `model_input_readiness_matrix.md`                    | 按 **data_domain** 拉 bar/公告/宏观；Layer1 **指标计算** → **G12** Round4+                                                                                 | 多数建模输入 **deferred**；非「指标清单逐条抓取」                    | `R3G_MASS_REHEARSAL_OPEN_GAPS.md` **G12**                                     |

#### 5.0.6.3 写库覆盖规则（upsert + 两道闸）— 通俗口径

**不是「日历同一天就无脑覆盖」。** 对日线 clean 表，主键是 **`instrument_id` + `trade_date` + `adjustment_type`**（不同复权类型是不同行）。

```text
外部拉数 → staging
    ↓
【闸 1 质检】DataQualityValidator — 格式/缺字段/离谱 OHLCV 等；FAILED → 拒写 clean
    ↓
【闸 2 对账】SourceConflictValidator — 多源严重冲突 → 拒写 + 人工复核队列
    ↓
【上架】WriteManager upsert_by_pk：删掉 clean 里与 staging 主键相同的行，再 INSERT staging 整行
         → 整行替换，不是字段级 merge
```

| 情况                          | 行为                 | 风险 / 缓解                                  |
| ----------------------------- | -------------------- | -------------------------------------------- |
| 同主键、同内容、重复跑任务    | 行数不增（写库幂等） | 任务级仍会重复下载（抓取层未 watermark 时）  |
| 同主键、源头修正、两道闸通过  | 新整行覆盖旧整行     | 设计意图；完整 **RevisionAudit** 仍待 Batch6 |
| 质检/冲突未过                 | **不写** clean       | `write_contract.yaml` gate                   |
| 程序 bug 产出坏数据但闸未拦住 | 可能覆盖好数据       | 靠 caps、审计、人工复核；非 upsert 单独能防  |

**两层「幂等」勿混：** **任务级**（要不要重新下载/建 job）与 **写库级**（同 PK 不叠行）是不同层；前者 Batch6，后者 R3H-06 已闭合。

#### 5.0.6.4 典型场景索引（例：baostock 十年日线 + 每日增量）

| 步骤                | 用户期望                             | 规划落点                                          | 当前                                                       |
| ------------------- | ------------------------------------ | ------------------------------------------------- | ---------------------------------------------------------- |
| 1. 首次灌 10 年历史 | FullLoad 或分段 Backfill             | **Batch6** FullLoad/CLI + **R3H-08A** 真网 Tier A | FullLoad deferred；backfill 有 cap；`--live-wire` 仅 pilot |
| 2. 每日只拉新 bar   | Incremental + 读库 `max(trade_date)` | **R3H-08** 产品化 + **Batch6** watermark          | runner 有；自动读库窗口 **缺**                             |
| 3. 写入不重复叠行   | upsert_by_pk                         | **R3H-06**                                        | ✅ CLOSED                                                  |
| 4. 源头改历史价     | RevisionAudit → 局部 backfill        | **Batch6**                                        | 极简 runner only                                           |
| 5. 算 Layer1 指标   | 原材料 → 指标引擎                    | **G12** Round4+                                   | deferred                                                   |

#### 5.0.6.5 设计已有 / 还缺模块 / 是否进规划（速查）

| #   | 模块                                            | 设计        | 实现        | 进规划？             | 闭合批次                   |
| --- | ----------------------------------------------- | ----------- | ----------- | -------------------- | -------------------------- |
| 1   | 三域 clean 表 + promote 路由                    | ✅          | ✅ R3H-06   | ✅                   | R3H-06 **CLOSED**          |
| 2   | upsert_by_pk + 两道闸                           | ✅          | ✅          | ✅                   | R3H-06                     |
| 3   | Incremental runner 金路径                       | ✅          | ✅          | ✅                   | Round2 done                |
| 4   | Incremental 自动读库窗口                        | ✅          | ❌          | ✅ §5.0.6.2          | Batch6 + R3H-08            |
| 5   | Backfill 分片 + 每片写 clean                    | ✅          | ✅          | ✅                   | Round2 + R3-PARTIAL-1 余量 |
| 6   | FullLoad 产品 runner                            | ✅          | ❌ deferred | ✅ D2-P1-1           | Batch6                     |
| 7   | RevisionAudit 产品                              | ✅          | ⚠️ 最小     | ✅ D2-P1-1           | Batch6                     |
| 8   | 任务级 idempotency_key                          | ✅          | ❌          | ✅ Batch6            | Batch6                     |
| 9   | `quant_monitor.sync` / `qmd data sync` 生产 CLI | ✅          | ❌ D2-P1-3  | ✅                   | Batch6                     |
| 10  | Sync production-equivalent smoke                | ✅          | ❌          | ✅                   | **Batch05** §7.5           |
| 11  | 真网 baostock→主库（非 live-wire）              | ✅ R3H PASS | ⚠️          | ✅ **R3H-08A**       | Wave 3                     |
| 12  | DataSourceService 去双轨                        | ✅          | ⚠️          | ✅ **R3H-10**        | Wave 2                     |
| 13  | Layer1 指标计算（G12）                          | ✅          | ⚠️ staged   | ✅（不进 PASS 门禁） | Round4+                    |

**下游文档（细节 SSOT，本节不替代）：**

```text
docs/modules/data_sync_orchestrator.md      # 五类 job 行为与 §13.4 增量/回补算法
docs/modules/write_manager.md               # upsert、validation gate、事务边界
docs/ops/idempotency_retry_dlq_policy.md    # 任务级幂等与重试
...
docs/UNRESOLVED_ISSUES_REGISTRY.md          # R3-PARTIAL-1/3 等
ROUND3_BATCH_IMPLEMENTATION_MAP.md          # Batch6 条目全集
docs/implementation_tasks/ROUND_3_BATCH6_DATA_GOVERNANCE/BATCH_3F_BATCH6_DATA_GOVERNANCE/
R3H_PASS_EXECUTION_PLAN.md                  # LIVE-PROD / Tier A–C
```
...
Wave 0  Batch 3V（六卡 trust）                    【卡间并行；merge 串行】
  ↓
Wave 1  R3H-06 Clean Schema                       【**CLOSED** @ 2026-06-29】
  ↓
...
| ----- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- | ----------------------- | --------------------------------------------------------------------------------------------------------------------- |
| H-A   | `R3H_01_OFFICIAL_MACRO_DISCLOSURE_ADAPTERS.md`       | `fred`, `us_treasury`, `sec_edgar`, `cftc_cot`, `bis`, `world_bank`                                                           | **CLOSED** @ 2026-06-28 | 六源 `READY_WITH_EVIDENCE`；Layer1/L5 smoke；**G10 + G14（FRED）已闭合**（audit PASS）。                              |
| H-B   | `R3H_02_MARKET_DATA_ADAPTERS.md`                     | `alpha_vantage`, `stooq`, `yahoo_finance`, `deribit`, `coingecko`                                                             | **CLOSED** @ 2026-06-28 | 五源 `READY_WITH_EVIDENCE`；yahoo `validation_only` 永久；**US 日历仍自然日窗** → R3H-05 §5.0.1 **CAL-US**。          |
| H-C   | `R3H_03_CN_MARKET_ADAPTERS.md`                       | `baostock`, `akshare`, `cninfo`, `tdx_pytdx`, `mootdx`, `eastmoney`, `sina_finance`, `ths_ifind`, `qmt_xtdata`, `qmt_xqshare` | **CLOSED** @ 2026-06-28 | 十源终态 + audit 零遗留；**CN G2/G17 日历已闭合**（Q12）；G11/G16 live 产品化 → R3H-05。                              |
| H-D   | `R3H_04_PREDICTION_AND_WEB_EVIDENCE_ADAPTERS.md`     | `kalshi`, `polymarket`, `web_search`                                                                                          | **CLOSED** @ 2026-06-28 | 三源 evidence/probability/manual-review；**web_search mock stub**（真 API 延后）→ R3H-05 §5.0.1 **WEB-SEARCH-LIVE**。 |
...
| Sync orchestration production-equivalent smoke        | 历史底座 + 3H/3G 输入；分阶段索引 **§5.0.6** | Batch05     |
...
| Data health engine                             | G14：FRED 官方宏观已闭合；baostock pilot 仍用 bridge DH sidecar             | Batch3F-R / R3FR-02 + **R3H-01（FRED）** + R3H-03/05（余量）           | Batch05          | EasyXT-style profiles；取消 baostock bridge sidecar 归属 R3H-03/05。                                             |
...
| Sync orchestration                             | 五类 job / 读库窗口 / 任务幂等 / CLI 见 **§5.0.6**；production smoke 未闭合 | Round2 骨架 + **R3H-08** 产品化 + **Batch6** repay + **Batch05** smoke | Batch05          | incremental/backfill/reconcile bounded smoke，经 DataSourceService；FullLoad/RevisionAudit/CLI 主能力在 Batch6。 |
...
13. 若涉及 **FullLoad / Incremental / Backfill / RevisionAudit、增量读库、写库覆盖**，是否先读本文 **§5.0.6**，再下钻 `data_sync_orchestrator.md` 与 `014` / Batch6 任务卡？
