...
| grill-me 扩批        | **6/7 AC 可勾选** — AC-4 **⏸ 合并 master 后** `workflow_dispatch`                   |
...
| grill AC 七条                    | **6/7 可勾选**（AC-4 ⏸ merge→main 后） | `task.json` → `review`                                          |
...
| **G-05** | 生产前置未验：无二次幂等、无 CI 实跑、沙箱≠生产未入 ADR/MCR、B/C 未覆盖     | §9 #3–#7                                                            | AC-3 · AC-4 · AC-6 · AC-7 | **✅ 部分** · 幂等+B/C+ADR 已证 · AC-4 ⏸ merge 后        |
| **G-06** | 任务未关：`in_progress`；grill AC **0/7**                                   | handoff 门禁 + `task.json` → `review`（**非** finish-work）         | §2.3 T-14/T-15            | **✅ 部分** · `review` · 6/7 AC                          |
...
| AC-4 | ⏸ **合并 master 后** — 缺 GitHub `workflow_dispatch` run URL                                                                   |
...
| grill **AC 1–7**                               | **6/7**        | §2.1 · §5 · §2.2 AC 表        | AC-4 待 GH dispatch |
...
| T-09 | CI **workflow_dispatch** 实跑 11/11 + artifact                               | 原缺口#5 · full_triple · G-05b | AC-4 · §9#4  | ⏸ **合并 master 后**                      |
...
| T-15 | grill AC **6/7 可勾选** + `task.json` → `review`（**非** finish-work）       | G-06                           | §9#9         | ✅ 6/7 · AC-4 ⏸                           |
...
| **AC-4** | CI 实跑：`tier-a-live.yml` workflow_dispatch 全量 11/11 可核对                                           | GitHub Actions run URL 或本地 `act`/等价日志 + artifact 路径                           |
...
| **4**  | **G-05b**：CI workflow_dispatch 实跑                                                   | 稳定              | **⏸ 合并 master 后** · AC-4                                                             |
...
| **9**  | `task.json` → `review`（**非** finish-work）/ commit                                   | T-01–T-17 除 T-09 | **[x]** 6/7 AC · 待 merge 后 AC-4                                                       |
...
_End of handoff — **rev 4.4** · 关账切片 2/2 · `task.json`=`review` · 6/7 AC（AC-4 ⏸ merge 后）· T-18 跳过。_
