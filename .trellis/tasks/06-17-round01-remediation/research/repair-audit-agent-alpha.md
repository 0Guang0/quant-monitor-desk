...
| **P0-3** | reader pragmas | **VERIFIED** | `backend/app/db/connection.py` L172–177: `reader()` calls `self._apply_pragmas(con)` before yield. Tests `test_reader_appliesThreadsAndMemoryLimit` / `test_reader_appliesTempDirectory` in `tests/test_duckdb_connection.py` L57–76. *(Note: reader pragma call existed at `3d7f93a`; repair added explicit GPT-named tests.)* |
