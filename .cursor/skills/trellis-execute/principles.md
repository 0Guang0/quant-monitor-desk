...
5. **分级遗留**：清单中任一项（P0–P3、阻塞/非阻塞、high/medium/low、可优化、待改进等）未修复、未阶段外置，却以优先级低为由跳过
...
- **无遗留**（修复/收尾时）：逐项关闭；仅能 **阶段外置**（写清后续阶段/任务 ID 与依赖，本阶段 AC 或用户已接受）

**完成条件**：根因已消除，或用户已接受带截止的债务，或已 **阶段外置** 且绑定后续交付；不得留下无出口的绕过或 **无绑定的分级遗留**。
