...
| --------- | ------------------------------------------------- | ------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `R3FR-01` | `R3FR_01_REFERENCE_RULES_AND_LICENSE_GATE.md`     | **Done** — reference-adoption governance re-executed; task-local execution mandatory                   | `reference_adoption_guardrails.yaml`, roadmap/task indices, `MODULE_COMPLETION_RATING.md`, `PRODUCTION_COMPLETION_VERTICAL_SLICE_PLAN.md` | No central executable inventory; production plan is coverage map only |
| `R3FR-02` | `R3FR_02_EASYXT_DATA_HEALTH_REFACTOR.md`          | **Done** — `market_bar_p0` read-only profile engine with EasyXT-inspired rule closure                  | `backend/app/ops/data_health_profiles/**`, `data_health.py`, `data_quality_rules.yaml`                                                    | Complete `market_bar_p0`, not one-rule increments                     |
| `R3FR-03` | `R3FR_03_TDX_PROVIDER_REFACTOR.md`                | **Done** — disabled/raw-only `TdxPytdxFetchPort` + gate-only live auth merged @ `22086d59`             | `backend/app/datasources/fetch_ports/tdx_pytdx_port.py`, `normalizers/tdx.py`, probe/registry tests                                       | Complete disabled/raw-only provider shape                             |
| `R3FR-04` | `R3FR_04_JQ2PTRADE_BACKTEST_ADOPTION_PLAN.md`     | **Done** — Round4 backtest/review cards rewritten with JQ2PTrade/EasyXT adoption detail                | Round4 `B04_05`, backtest/review contracts                                                                                                | Round4 must fit ≤3 implementation batches to full stable scope        |
| `R3FR-05` | `R3FR_05_PROVIDER_CATALOG_OPENBB_REFERENCE.md`    | **Done** — provider catalog SSOT (25 sources) + OpenBB architecture-only reference merged @ `6c1a0d37` | `provider_catalog.yaml`, provider catalog tests, source contracts, schema CHECK enum alignment                                            | Catalog fields and all registry entries in one batch                  |
| `R3FR-06` | `R3FR_06_QMD_DATA_HEALTH_CLI_RUNTIME.md`          | **Done** — `qmd data health` wired to shared `run_data_health_profile` read-only runtime               | CLI command files, data CLI contract/tests                                                                                                | Same batch as R3FR-02; no message-only wrapper                        |
| `R3FR-07` | `R3FR_07_LEGACY_WRAPPER_CLEANUP_AND_REDIRECTS.md` | **Done** — legacy wrapper redirect docs; Batch 3F-R closed; 3G unlocked                                | wrapper docs/tests, task indices, roadmap                                                                                                 | Cleanup cannot create more micro-slices                               |
