...
| Source-route 矩阵        | `uv run python scripts/qmd_ops.py accept-source-route-db --all-documented-sources` | 临时/受控 | ADR-016 closure + matrix checker           | `docs/decisions/ADR-016-source-route-matrix-honest-closure.md` |
| 生产等价 smoke（legacy） | `python scripts/acceptance_pipeline_smoke.py`（见 ADR-016；首选 matrix）           | 临时/受控 | cleanup/resource guard/schema              | `incident_playbook.md#production-equivalent-smoke`             |
