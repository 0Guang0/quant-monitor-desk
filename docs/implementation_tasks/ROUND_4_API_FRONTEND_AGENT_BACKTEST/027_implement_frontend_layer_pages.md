...
执行 frontend Layer pages / API contract 相关计划前，必须读取 `docs/implementation_tasks/UNRESOLVED_ITEM_TASK_COVERAGE.md`，并核对：

| ID              | 归属阶段               | 本任务卡处理要求                                                          |
| --------------- | ---------------------- | ------------------------------------------------------------------------- |
| `R3-B25-HYG-02` | Round4 task 027        | 前端 Vitest 与 bundle budget gate；Round3 仅 cross-reference，不得遗忘。  |
| `R4-API-SEC-5`  | Round4 tasks 024 + 027 | frontend/API stale page-size contract test。                              |
| `R4-FE-3`       | Round4 task 027        | Layer 1–5 dashboard pages not implemented；必须实现或 explicit re-defer。 |
