...
|----|----------|----------|-----------------|------|------|------------------|--------------------------|
| **A1** | Spec 合规、依赖完整 | read-only | trellis-check；diff vs check.jsonl；GitNexus 未声明依赖 | local | 无写 | 无未授权偏离 | **必须**找到 ≥1 个 implement.jsonl 未声明但代码引用的模块？找不到则说明为什么 |
...
| **A5** | AC 完成度、证据可信 | trace-ac | 逐条 MASTER §2 ↔ 验证链；1–5 分 | local | 无写 | AC 均可追溯 ≥4 | **必须**挑 1 条最可疑的 Execute 证据，说明为什么可疑 |
...
| **A8** | 测试缺口 | pytest-isolated | 补 §7 Red Flags / 边界用例；**可追加 2 个自选边界用例** | audit-sandbox | tmp DB / `@audit` marker | 新测绿或入 §4.3 | MASTER §7 的 Red Flag 是否全被覆盖？至少补 2 个 |
