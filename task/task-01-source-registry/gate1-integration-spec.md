...
> **状态：** ready-for-agent（设计已确认；实际候选优先级与回补窗口先从受控配置核实）
> **权威输入：** ADR-017、`source_provenance_quality_contract.yaml`、Source Registry／RoutePlan／WriteManager 设计。
...
**输入：** platform、data_domain、operation、frequency、授权上下文，以及只读的有效启用状态。
**输出：** 持久化 RoutePlan：候选顺序、selected source/role、route status、机器原因码、来源等级、质量等级、人工复核、主源失败原因、registry/overlay revision 与恢复关联。
...
| ------------------------ | ------------------------------------------------------------------- | ------------ | ----------------------------------------------------- |
| G1-01 接线清单与配置核实 | 列出所有正式入口、已有 fallback 顺序/窗口、责任与同参命令。         | 无           | 清单没有 `VERIFY_REQUIRED` 的未证实结论。             |
| G1-02 task-01 策略接缝   | 有效启用 + capability 查询成为唯一策略入口；直接 CLI/宏观绕过删除。 | G1-01        | 允许／未授权／能力缺失三种同参测试。                  |
