...
| --------------------------- | ---------- | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ACC-MOOTDX-DRYRUN-ROUTE-001 | 2026-07-02 | mootdx dry-run `selected_source_id` 与 `--source-id mootdx` 对齐；去掉 runtime `validation_only` hack | `source_registry.yaml` mootdx `validation_only: false` · `data_commands.py` 无 `__setattr__` · `test_tierASyncRouter_dryRun_mootdx_selectedSourceId_aligned` · ADR-014 |
...
| ACC-MIGRATION-MATRIX-PATH | 2026-07-01 | `test_migration_coverage` historical archive path          | `docs/schema/MIGRATION_COVERAGE.md`                                                                                                                                                                          |
...
| D3-P1-2                   | 2026-07-09 | C901 write-path — S7 提取式简化关账                        | `docs/decisions/ADR-004-write-path-complexity-ceiling.md` · `ruff --select=C901` · `tests/test_write_manager.py` · `tests/test_source_registry.py`                                                           |
...
| ID           | Closed     | Item                                            | Evidence                                                                                                                                                       |
| ------------ | ---------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| R3-TASK-023  | 2026-06-25 | Full Layer5 evidence chain (`023b` / Wave D)    | merge `376e30e6` · `backend/app/layer5_evidence/evidence_chain.py` · `tests/test_layer5_evidence_chain.py` · `docs/adr/ADR-023-layer5-conflict-review-path.md` |
...
| ID                  | Closed     | Item                                                 | Evidence                                                                                                                            |
| ------------------- | ---------- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| R3-TASK-021         | 2026-06-24 | Layer3 industry-chain daily snapshot (`021`)         | `backend/app/layer3_chains/snapshot_builder.py` · Audit PASS · 8 snapshot tests                                                     |
| R3-PROMPT19-V2      | 2026-06-24 | PROMPT_19 staged real-data pilot v2                  | merge `e4abb372` · `tests/test_staged_pilot.py` · Audit PASS                                                                        |
...
| R3Y-REGISTRY-ALPHA2 | 2026-06-24 | fix alpha-2 registry / COVERAGE / Map §2.4 reconcile | merge `984c7b28` · `tests/test_round3_audit_registry_alignment.py` · `tests/test_unresolved_item_task_coverage.py`                  |
| R3Y-SYNC-001        | 2026-06-24 | Sync production `adapter=` bypass fail-closed        | merge `616feeb8` · `tests/test_sync_orchestrator.py::test_r3ySync001_*`                                                             |

## Post-PROMPT_18 wave-A resolved (2026-06-23)

| ID                           | Closed     | Item                                                    | Evidence                                                                 |
| ---------------------------- | ---------- | ------------------------------------------------------- | ------------------------------------------------------------------------ |
| R3-B3-STAGED-DOWNSTREAM-GATE | 2026-06-22 | Batch 3 staged-only downstream gate closed (docs/tests) | `docs/quality/BATCH3_STAGED_DOWNSTREAM_GATE.md`                          |
| R3-TASK-019                  | 2026-06-23 | Layer2 cross-asset sensor (`019`) staged pipeline       | Audit PASS                                                               |
| R3-TASK-020                  | 2026-06-23 | Layer3 industry chain loader (`020`)                    | `backend/app/layer3_chains/` · Audit PASS · 14 tests                     |
| R3-TASK-023A                 | 2026-06-23 | Layer5 evidence foundation (`023A`) minimal slice       | Historical task evidence                                                 |
| R3Y-AUDIT-GATE-18            | 2026-06-23 | PROMPT_18 post-R3X strict adversarial audit complete    | Historical go/no-go evidence · **`WARN_ALLOW_WITH_CONTROLS`** (no BLOCK) |
...
| ID                    | Closed     | Item                                                                                                                                                       | Evidence                                                                                                                       |
| --------------------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ |
| R3-PROMPT14-STAGED-01 | 2026-06-22 | PROMPT_14 staged real-data pilot executed; closeout `PILOT_PASS_STAGED_RAW` (partial — akshare validation NETWORK_ERROR; see `R3-PROMPT14-AKSHARE-VAL-01`) | `docs/quality/prompt14_user_authorization_2026-06-22.md` · `tests/test_staged_pilot.py`; does **not** close `R3-B2.75-REQ2-EM` |
...
| DB-R3-001                 | 2026-06-20 | Project `data/` root inventory via inspect     | db-inspect evidence (`raw_files_count`, `parquet_files_count`); documented absence/limited local files acceptable per Batch 1 AC.                                          |
| DB-R3-002                 | 2026-06-20 | Read-only DuckDB open + key table row counts   | db-inspect evidence (`read_only_open=true`, `key_tables`); `tests/test_ops_db_inspector.py::test_dbInspect_dbFile_unchanged`.                                              |
...
| R3-EARLY-PROD-SCALE-BENCH | 2026-06-20 | Production-equivalent smoke evidence (Batch 1) | `scripts/production_equivalent_smoke.py --use-service-path` (cross-ref `R2.6-IMPL-7`).                                                                                     |
...
| R2.6-B1     | 2026-06-19 | Round2.6 Phase B contract gate tests                    | `tests/test_source_capabilities.py`; `tests/test_source_route_planner.py`; `tests/test_datasource_service.py`; `tests/test_module_boundaries.py`; `tests/test_qmd_data_cli.py`; `tests/test_dependency_extras_contract.py`; `tests/test_platform_source_matrix.py`. |
| R2.6-B2     | 2026-06-19 | Module boundary static checker                          | `scripts/check_module_boundaries.py`; `tests/test_module_boundaries.py`.                                                                                                                                                                                            |
| R2.6-B3     | 2026-06-19 | Phase A self-check migrated to historical task evidence | historical routing-service gate evidence.                                                                                                                                                                                                                           |
...
| PROC-R2.5-1 | 2026-06-19 | Round2.5 merge + historical handoff                                | PR #15 → commit `7ce283a` on `master` per registry.                                                                                              |
| D3-P3-1     | 2026-06-19 | Five vendor skeleton adapter classes                               | `tests/test_adapter_skeletons.py`.                                                                                                               |
| D5-P1-2     | 2026-06-19 | Manifest protocol uses historical archive                          | frozen Batch D archive.                                                                                                                          |
...
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `pytest tests/test_production_live_pilot_policy.py tests/test_batch25_production_data_gate.py tests/test_global_execution_rules.py tests/test_project_scaffold.py -q` | passed in historical session.                                                                             |
...
| `pytest tests/test_global_execution_rules.py tests/test_project_scaffold.py -q`                                                                                       | passed in historical session.                                                                             |
