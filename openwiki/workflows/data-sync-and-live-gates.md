...
`backend/app/datasources/live_prod_source_tiers.py` maps source IDs to production tiers (incremental gold path / Tier B / Tier C). The incremental gold path currently includes `fred`, `us_treasury`, `sec_edgar`, `cftc_cot`, `bis`, `world_bank`, `alpha_vantage`, `deribit`, `baostock`, `cninfo`, and `mootdx`. Tier B contains sources such as Yahoo, AkShare, Stooq, CoinGecko, Eastmoney, Sina, TDX, iFinD, QMT, and qmt_xqshare. Tier C contains Kalshi, Polymarket, and web search.
...
- `backend/app/datasources/live_prod_source_tiers.py`
