...
| B2.5-O-07 | Double `fetch_log` row per `DataSourceService.fetch` (adapter + service layers)       | **DataSourceService refactor** (optional)     | `service.py` + `base_adapter.py`               | No              | `+2` delta documented; **lineage authoritative row** = `ORDER BY fetch_time DESC LIMIT 1` in `data_quality.py::_collect_fetch_lineage` (Phase 4 closed) |
