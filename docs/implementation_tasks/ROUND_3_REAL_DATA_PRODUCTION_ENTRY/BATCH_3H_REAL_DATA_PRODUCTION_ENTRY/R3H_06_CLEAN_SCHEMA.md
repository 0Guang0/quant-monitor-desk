...
| 选项                                                 | 结论                                             |
| ---------------------------------------------------- | ------------------------------------------------ |
| A. 继续遗留 bar CTAS 无 PK                           | **否决** — G3/G4/G6 未闭合                       |
...
| pilot 兼容      | **无 VIEW** @ 用户 2026-06-29 — 删除遗留 bar CTAS；实现路径统一 `security_bar_1d` |
...
- **预演偏离：** `research/r3g03_mass_rehearsal_gap_analysis.md` §3 — 三源同表遗留 CTAS。
...
| 9.5  | domain_router  | 新建 `clean_write_targets.py`：`domain→(target_table, staging_table, write_mode, primary_keys)`；`limited_production_entry` **删除**硬编码 `append_only`/遗留 bar CTAS |
...
| 9.8  | pilot_compat   | 实现路径零遗留 bar CTAS 表名（见 §9.8.1）；改 `r3g03` 脚本/测/fixture→`security_bar_1d`                                                                                |
...
### 9.8.1 遗留 bar CTAS 表名清除门禁（可执行）

在 `backend/`、`scripts/`、`tests/`、`specs/`、`docs/implementation_tasks/ROUND_3_REAL_DATA_PRODUCTION_ENTRY/`（排除 `.trellis/`）中搜索 R3G-03 遗留三源同表 CTAS 标识符；**零匹配**（ripgrep exit 1）为通过。可执行命令见 Trellis frozen `R3H_06_CLEAN_SCHEMA.md` §9.8.1。
