...
L2 (R3-DCP-01): fetch_port gold path still routes through adapter.fetch; staging
mapping required so run_incremental receives staging_table (see reference-adoption-dcp01.md).
"""
...
_BAOSTOCK_STAGING_TABLE = "stg_foundation_smoke"
...
class BaostockAdapter(SkeletonAdapterBase):
    ...
    def fetch(
        ...
        *,
        ...
        con.execute(f"DELETE FROM {_BAOSTOCK_STAGING_TABLE}")
        for bar in bars:
            close = float(bar.get("close") or 0.0)
            con.execute(
                f"""
                INSERT INTO {_BAOSTOCK_STAGING_TABLE} (
        ...
        return result.model_copy(
            update={"staging_table": _BAOSTOCK_STAGING_TABLE, "row_count": len(bars)}
        )
