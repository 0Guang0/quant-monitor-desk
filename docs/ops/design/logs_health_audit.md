...
| exception payload   | 见领域频率规则 | 见领域频率规则 | 主源回补验证、归档与审计索引完整前不得清理 |
...
用户已拍板：第一版对普通 `raw`、`audit`、`report`、`notification_log` 采用 1 年逻辑留存；可信最终库不自动清理，被回补替代的异常 payload 按领域频率和计算窗口留存。
...
cold: 普通内容归档到 365 天；
异常 payload: 按领域频率基线与最长计算窗口归档；
可信最终库: 不因本策略自动清理。
```

普通内容不得以其他留存期覆盖 D-05；异常 payload 的 90 天/2 年/5 年/7 年规则与可信最终库不自动清理是本策略的正式例外，不是历史说明。

必须审计启用覆盖层变更/撤销、Primary 失败、每个 fallback 候选尝试、连续监控写入、人工复核、
主源恢复、异常版本归档与到期清理。异常 payload 的频率规则、可信最终库不自动删除和精简审计
索引长期保留，以 `source_provenance_quality_contract.yaml` 为准。
