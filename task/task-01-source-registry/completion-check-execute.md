...
# completion-check · 追加轮次（票 06 / 07）

- 角色：`execute`
- 日期：`2026-07-12`
- 对应 plan：`task/task-01-source-registry/task_plan.md` 工作包 4a/4b；`g1-02-execution-brief.md` §2–§3.1 / §6 **4a/4b** / §7；票 `.scratch/task-01-g1-02-enable-seam/issues/06-migrate-incremental-override.md` · `07-migrate-cli-backfill-gold-path.md`
- 对象范围：票 **06**（E-INC-\* / BUNDLE / E-ACC-01 preview 去 ESR 撬门；沙箱正规 overlay→READY）∥ 票 **07**（E-CLI-20 金路径 fred+else + E-CLI-13 mootdx；acceptance/matrix 正规 overlay；「只清 fred 漏 else」反证）。**不含** 票 08 bridge、夹具零内存构造（F06）、删 `con=None` 回落（F07）、F08–F12 关账、FRED 编排合并（票 10）、G1-02 整包、模块 R4
- 声称：票 06 + 票 07 AC **Execute CLOSED**（≠ G1-02 CLOSED / ≠ R4 / ≠ F05-A「仅测债」升格 / ≠ 夹具零构造）
- 权威：ADR-018 · `docs/modules/design/data_sources.md` §5.2.1 · brief §2/§3.1/§6/§7 · 票 06/07 AC · `note.md` 06∥07 拍板 · findings F05-A（测债已关）/ F03 余 4x
- 正式入口：`load_incremental_route_bundle` · 各 E-INC 工厂 · `matrix_live_handlers` / `source_route_db_acceptance*` · `qmd-data data sync`（`_incremental_route_preview` / `_dry_run_shell`）· `qmd-data data backfill`（`_gold_path_backfill_route_preview` / `backfill_plan`）· mootdx CLI 预览 `plan(preferred_primary_source_id=)`
- 声称档位：`sandbox`（`.audit-sandbox` 正规 overlay + 平台矩阵夹具可 READY）+ `dry-run`（sync 不静默 ensure）+ `product_default`（无 overlay 同参仍非 READY / DISABLED_SOURCE）；**非** live 升格「产品已默认启用」

## 业务价值（对靶摘要）

| 票  | 交付                                                                               | 业务价值                                          |
| --- | ---------------------------------------------------------------------------------- | ------------------------------------------------- |
| 06  | 增量/验收不再内存撬门；沙箱 overlay 可 READY；产品默认仍诚实关                     | 运维/验收路径与问开关同路；假开源不再冒充产品默认 |
| 07  | backfill 金路径 fred **与** else 均只读问开关；mootdx 钉主源；只清 fred 则静态门红 | 回补不会「清一半还绿」；显式 `--source-id` 可审计 |

## 逐对象关账记录

### 对象 F · 票 06（4a · E-INC / BUNDLE / acceptance preview）

| CC        | 具体场景示范                | 本对象运行事实                                                                                                                                                                                                                                                                                        | 证据 / 反证                                                                                                                                                                         | Verdict | 闭环控制                                                     |
| --------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- | ------------------------------------------------------------ |
| CC-0 对靶 | 只报 pytest 绿 / 升格 G1-02 | 用户要：E-INC-\*+BUNDLE+E-ACC-01 去 ESR/强制 platform；沙箱 overlay→READY；产品默认同参仍关；**不**关 FRED 合并、**不**关 F06 零构造。交付即 `load_incremental_route_bundle` 干净 load、`ensure_sandbox_route_activation` / `build_activation_route_planner`、生产无 `enabled_*_source_registry` 定义 | 票 06 AC；brief §6 4a；`macro_incremental_common.load_incremental_route_bundle`；`incremental_route_activation.py`                                                                  | PASS    | 关账仅票 06；升格 G1-02/R4/F06 零构造须新 claim              |
| CC-1 证伪 | 静态存在即完成              | 覆盖集合：①沙箱 overlay+bundle→READY+revision（fred）②产品默认库同参非 READY（fred）③A1–A4 e2e 旧口径测绿（overlay 夹具）④生产扫描无 ESR 字样。反证：无 overlay 产品库仍 DISABLED；`--strict` hygiene 人为加回 `enabled_*`/`_platform_allows=` 必红                                                   | `uv run pytest -q tests/test_g1_02_incremental_route_activation.py`；A1–A4+A 共担 node-id 定向 exit 0（2026-07-12）；`phase-scripts/check_g1_02_esr_fixture_hygiene.py --strict` OK | PASS    | 再引入生产 ESR 定义或无 overlay 仍 READY → 测/hygiene 必须红 |
| CC-2 验真 | 只删工厂名仍内存启用        | 原子：干净 `SourceRegistry.load`；启用经 `ask_activation(con=)`；沙箱写 overlay+平台矩阵；acceptance/matrix live 走 `ensure_sandbox`+`build_activation`；`fred_incremental_watermark` 无 `enabled_fred_*`。FRED **编排合并**未做（诚实外置票 10）                                                     | Grep `backend/app`：无 `def enabled_\w+_source_registry` / `planner._platform_allows =`；watermark 注释与实现                                                                       | PASS    | 若声称「FRED 编排已合并 / 夹具零内存」→ FAIL                 |
| CC-3 同路 | CLI READY、scheduler 仍 ESR | 声称入口（E-INC/BUNDLE/ACC preview）均经干净 bundle/activation planner + `plan(con=)`。票 **08** bridge / 误导别名清理为范围外；本轮生产别名定义已删。测试 `registry_factory=load_plain_source_registry` 与正式干净 load 同语义                                                                       | inventory E-INC-\*；hygiene PROD_ESR_PATHS；sync/acceptance 调用链                                                                                                                  | PASS    | 若声称「含 4x bridge 全入口已同路」→ FAIL（→票 08）          |
| CC-4 验档 | sandbox READY 升格产品启用  | 证据分档：sandbox overlay READY；product_default 无 overlay 非 READY；sync dry-run **不**静默 ensure（未启用→DISABLED）。未标 live product-enabled                                                                                                                                                    | `test_loadIncrementalRouteBundle_productDefault_fredDisabled`；`test_incrementalSyncRouter` bis DISABLED；note 拍板                                                                 | PASS    | 禁止用 sandbox READY 声称产品默认已开宏源                    |
| CC-5 对表 | ADR 禁撬门，工厂仍 setattr  | 形：无 ESR/强制 platform；义：先问开关再安检；行为：沙箱 ensure 仅 `.audit-sandbox`；效果：A1–A4 业务 outcome 绿且 hygiene 静态度过。F09 域默认关+overlay 代码已改但**本 claim 不写 F09 已修复**                                                                                                      | ADR-018；票 AC；findings F09 待复验                                                                                                                                                 | PASS    | 偏差须事前审阅；本轮未改 `**/design/**`                      |
| CC-6 清债 | 能修却无限延后              | 本轮闭环：生产 ESR 撬门；死 helper；preferred 贯通相关债（联审）。严格后置（双登记、**不支撑**本 claim）：F06→票 08 后；F07→票 08 后；F03 余 4x→票 08；F08/F10/F11→票 09 前；F12→票 09；F09→票 09 复验；FRED 合并→票 10                                                                               | findings · 待修复清单 · ROADMAP · note disposition                                                                                                                                  | PASS    | 后置项不得用来声称 G1-02 / 夹具零构造 / 无 con 回落已删      |
| CC-7 守闸 | 隐瞒 F06/合并未做           | 诚实未完成：F06 夹具内存构造、F07 回落、4x bridge、F08–F12、FRED 合并、G1-02 整包、R4、管理员写 overlay CLI                                                                                                                                                                                           | HANDOFF / findings 开放表                                                                                                                                                           | PASS    | CC-7 PASS≠G1-02；更大对象仍 OPEN                             |

### 对象 G · 票 07（4b · E-CLI-20 全金路径 + E-CLI-13）

| CC        | 具体场景示范                        | 本对象运行事实                                                                                                                                                                                                                                                                                              | 证据 / 反证                                                                                               | Verdict | 闭环控制                                                       |
| --------- | ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- | ------- | -------------------------------------------------------------- |
| CC-0 对靶 | 只清 fred 壳就关账                  | 用户要：`_gold_path_backfill_route_preview` fred **与** else 均问开关+安检；mootdx 钉主源；「只清 fred 漏 else」红；A5/A6 acceptance。未把 G1-02/R4 冒充本票                                                                                                                                                | 票 07 AC；brief E-CLI-20；`data_commands._gold_path_backfill_route_preview`                               | PASS    | 关账仅票 07                                                    |
| CC-1 证伪 | 业务测绿但 else 仍 ESR              | 覆盖：①非 fred 沙箱 overlay→READY（baostock）②preferred_primary 钉 mootdx（双源允许时）③backfill READY 且 `selected!=source_id`→CliFailure（去 `or source_id` 掩盖）④A5/A6 + layer5 共担绿⑤静态门：fred/else 体均无 ESR marker。反证：hygiene 拆 else 加回 ESR 字样必红；preferred 测缺钉则选 baostock 必红 | `uv run pytest -q tests/test_g1_02_gold_path_overlay.py`；A5/A6/layer5 定向 exit 0；hygiene `--strict` OK | PASS    | 再漏 else ESR 或去掉 preferred/fail-closed → 测/hygiene 必须红 |
| CC-2 验真 | 金路径仍半段 ESR                    | 原子：非 fred→`build_activation_route_planner`+`plan(preferred_primary_source_id=source_id)`；fred 仍走编排 preview 壳（**启用**不经 ESR）；mootdx CLI `preferred_primary_source_id="mootdx"`；acceptance/matrix `ensure_sandbox`+activation planner。编排合并仍票 10                                       | 金路径源码；`incremental_sync_router` fail-closed；acceptance 调用                                        | PASS    | 若声称「fred 编排已并入通用宏路径」→ FAIL                      |
| CC-3 同路 | backfill READY、sync 仍错选 primary | sync dry-run 与 backfill 均 `preferred_primary` + READY 选源不一致则 CliFailure。产品默认无 OVERRIDE 冒充：无静默 ensure。E-CLI-01/E-SVC-01 产品默认 DISABLED 由既有 planner/service + 本轮无 ESR 保证；本票未新开第二套启用权威                                                                            | sync/backfill 源码对称；bis dry-run DISABLED                                                              | PASS    | 再出现 payload `selected or source_id` 掩盖 → FAIL             |
| CC-4 验档 | dry-run/sandbox 升 live             | 证据为 sandbox overlay / dry-run 审计 JSON；未标 live 产品默认启用。金路径反证静态门在 phase-scripts（artifact-guard），**不**单独当 R4 证据                                                                                                                                                                | 测 docstring + hygiene 模块头；brief §7.6                                                                 | PASS    | 禁止把 hygiene OK 写成 G1-02/R4                                |
| CC-5 对表 | 权威要全金路径，实现只清 fred       | 形：fred 分支 + else 分支均无 ESR marker；义：只读问开关；行为：preferred 钉显式 source；效果：mootdx selected=mootdx。note 拍板：sync 不静默 ensure、backfill 沙箱可 ensure                                                                                                                                | hygiene `check_gold_path_esr_zero`；gold path 测                                                          | PASS    | 再只清 fred 漏 else → hygiene FAIL                             |
| CC-6 清债 | 掩盖选源能修却挂起                  | 本轮修：preferred 贯通金路径/sync；backfill fail-closed；去 `or source_id` 掩盖；删 sync 死回落。余债同对象 F 严格后置表                                                                                                                                                                                    | code-review 闭环 + pytest                                                                                 | PASS    | 同 F：后置不支撑本 claim 以外对象                              |
| CC-7 守闸 | 假装 G1-02 已关                     | 诚实未完成：同 F（08/09/10、F06/F07、G1-02、R4）                                                                                                                                                                                                                                                            | HANDOFF Frontier→08                                                                                       | PASS    | CC-7 PASS≠G1-02                                                |

## Summary（本追加轮 · 票 06∥07）

- 首个决定性缺口：none（关账前已修 preferred 贯通、backfill fail-closed、生产 ESR 清零、联审死代码；余债严格阶段后置且不支撑本 claim）
- 最终状态：`CLOSED`
- 声称结论：`permitted`（**仅**票 06 AC + 票 07 AC）
- 闭环控制：再验 =
  1. `uv run python phase-scripts/check_g1_02_esr_fixture_hygiene.py --strict`
  2. `uv run pytest -q tests/test_g1_02_incremental_route_activation.py tests/test_g1_02_gold_path_overlay.py tests/test_qmd_data_incremental_sync_router.py`
  3. findings F05-A 定向 A1–A6 命令（exit 0）  
     人为：生产路径加回 `enabled_*_source_registry` / `_platform_allows=`、去掉 preferred 或 `selected!=source_id` 检查、无 overlay 产品库仍 READY → 必须红。  
     **不得**将本记录当作 G1-02 / R4 / F06 零构造 / F07 回落已删 / FRED 合并 / 票 08 bridge 关账。

## 测试资产治理（本追加轮）

| 资产                                                                      | 角色                                      | 处置                                                      |
| ------------------------------------------------------------------------- | ----------------------------------------- | --------------------------------------------------------- |
| `tests/test_g1_02_incremental_route_activation.py`                        | 核心 / 增量 bundle READY+产品默认拒       | 保留                                                      |
| `tests/test_g1_02_gold_path_overlay.py`                                   | 核心 / 金路径 else READY + preferred 反证 | 保留                                                      |
| `phase-scripts/check_g1_02_esr_fixture_hygiene.py`                        | artifact-guard / phase-guard              | **保留在 phase-scripts**；有退役条件；**不得**单独证明 R4 |
| A1–A6 e2e / acceptance 旧测                                               | 核心业务 outcome（启用方式已迁 overlay）  | 保留；F05-A 测债已关                                      |
| `tests/service_path_support.enable_source_route` 内存域/capability 构造   | 测试基础设施噪音残留                      | **暂留** → F06 → 票 08 后；**不**作「零构造」完成证据     |
| sync `_enable_source_registry` → `prepare_audit_sandbox_route_activation` | 正式入口同路夹具                          | 保留（已非 monkeypatch ESR）                              |
