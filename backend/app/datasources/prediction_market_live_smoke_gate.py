"""Prediction-market capped live smoke gate (datasources layer · R3H-04/R3H-08).

Ops entrypoints import from here; ports must not import ops for product live paths.
"""
...
def run_prediction_market_live_smoke(
    *,
    ...
    """Capped live smoke for kalshi or polymarket (smoke env + YAML gated, not product live)."""
    validate_live_smoke_gate(source_id=source_id, authorization_path=authorization_path)
    if source_id == "kalshi":
        from backend.app.datasources.fetch_ports.kalshi_port import fetch_kalshi_smoke_live_payload

        payload = fetch_kalshi_smoke_live_payload(instrument_id)
        run_id = "r3h04-kalshi-live-smoke"
    elif source_id == "polymarket":
        from backend.app.datasources.fetch_ports.polymarket_port import (
            fetch_polymarket_smoke_live_payload,
        )

        payload = fetch_polymarket_smoke_live_payload(instrument_id)
