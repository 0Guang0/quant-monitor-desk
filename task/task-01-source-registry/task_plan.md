...
> **状态：** IN PROGRESS — Gate 0 已由用户确认并推广为索引设计：
> [ADR-017](../../docs/decisions/design/ADR-017-dynamic-source-fallback-and-exception-data-lifecycle.md)。
> 设计→运行副本 parity 已通过；[Gate 1 接线规格](gate1-integration-spec.md) 已建立，G1-01
> 配置核实与正式入口清单可立即执行，跨模块切片 4c～4e 仍不可关账。
>
...
- `docs/modules/design/data_sources.md`：数据源模型、角色、默认策略及同步要求。
...
这不是新增 HTTP API，而是 Task-01 向下游提供的稳定**策略查询接口**。它的产品含义很简单：
无论用户从服务、命令行还是定时任务请求某类数据，系统都应回答同一个问题——“此时能否用哪个
数据源；如果不能，为什么”。

| 契约项 | Task-01 必须提供 | 下游 task-02/17/18 必须遵守 |
|---|---|---|
| 输入 | 平台、领域、操作、频率（如适用）及经 Gate 0 批准的授权/启用上下文。 | 传递真实业务请求；不得私设 `force_enable`、伪造平台或替换 registry。 |
| 输出 | 复用现有 RoutePlan / 错误码设计的统一决策：可选 source（仅允许时）、状态、机器可读原因码、给用户的说明、策略版本/配置指纹。 | 只执行这个决策；禁止根据内部字段猜测、重写 source、把拒绝转成默认成功。 |
| 拒绝语义 | `USER_AUTH_REQUIRED`、禁用、能力缺失、平台不支持等必须可区分且在所有入口一致。 | 原样透传或按 `ERROR_CODE_GUIDE` 转译；不得改成含混的成功/空结果。 |
| 变更规则 | 一次只存在一个启用策略版本；新增字段应为向后兼容的附加信息。 | 不依赖未承诺的排序、日志文字或内部对象可变性（Hyrum 风险）。 |
| 测试/夹具 | 夹具可以在**构造策略输入之前**声明一份受控测试配置，但不得改写已加载的生产对象或平台判定。 | 将 fixture/dry-run 明确标为非生产档位，不能拿作发布放行证据。 |

Gate 0 已把该契约写入权威设计。G1-01～G1-05 必须把它落实为项目现有类型、错误码和配置字段的
精确契约；若现有 RoutePlan 字段不足，只能**新增兼容字段**，不得删除或改义已有字段。接口交付
前先枚举已有消费者与其可观察行为，避免因错误码文本、候选排序等隐性依赖造成回归。
...
G1-01、工作包 1 与 2 可并行；工作包 4a、4b 可并行；4c～4e 必须按 Gate 1 的阻塞边顺序完成。
...
- **依赖：** 0、1；**问题：** T01-F03 的根因。
- **预计触及：** `backend/app/datasources/source_registry.py`、
  `backend/app/datasources/service.py`、`backend/app/datasources/route_planner.py`、
  经 Gate 0 批准的启用配置/存储、`tests/test_datasource_service.py`。
- **RED：** 验证未授权/未启用不能被运行时对象突变放行；验证已批准的启用状态经唯一接缝被
  所有查询稳定读取，含原因与审计信息。
- **GREEN：** 只实现 Gate 0 批准的持久化/受控表达；保持 source registry 载入数据不可由
  消费者直接篡改。不得给 CLI/任务暴露任意 `force_enable` 或平台跳过开关。
- **验收：** 没有生产路径需要 `object.__setattr__`、替换 registry 查询函数或覆盖平台许可
  函数来执行；授权缺失始终失败关闭。
- **测试资产处置：** `tests/service_path_support.py` 及同类 helper 不能再通过 patch 已加载的
  registry/capability/platform 对象制造绿灯；改写为“在构造策略输入前提供受控配置→走正式入口→
  断言业务结果/原因码”的 contract 测试，或删除重复 helper。不得保留为本票关账证据的
  phase-guard / meta-test。
- **验证：** 服务与路由的集成测试，以及生产目录静态回归测试（禁止上述绕过模式）；人为移除
  正式授权配置或重新引入 override 时，测试必须在状态/原因码这个可观察点可靠变红。

### 3A. 策略决策的可观测性（与工作包 3 一并交付）

先回答运维真正需要回答的四个问题，再决定最小埋点，不为“多打日志”而加噪声：
...
| 信号 | 最小内容 | 约束与验收 |
|---|---|---|
| 结构化日志 | 稳定事件名 `source_policy_resolved` / `source_policy_denied`；包含 correlation/request id、平台、领域、操作、结果、原因码、策略版本/指纹。 | 复用项目现有 logger；不记录授权 token、密码、完整配置或用户隐私数据。 |
...
- **依赖：** 3；**问题：** `macro_incremental_common.py`、FRED 增量辅助路径。
- **预计触及：** `backend/app/ops/macro_incremental_common.py`、
  `backend/app/ops/fred_incremental_watermark.py`、对应测试（最多 4 个逻辑文件）。
- **RED/GREEN：** 先锁定无授权时不执行、经正式启用策略后可执行；再删除对象字段、角色函数和
  平台判断的动态覆盖，改为只使用工作包 3 的接缝。
- **验收与验证：** 两个正式任务在无授权时输出可操作原因，在隔离的受控已授权配置下通过路由；
  测试夹具不能修改生产 registry 对象。

### 4b. 迁移 CLI 消费者 — S

- **依赖：** 3；**问题：** `backend/app/cli/data_commands.py` 的回填/预览例外。
- **预计触及：** `backend/app/cli/data_commands.py`、其 CLI 测试、路由测试（最多 3 个逻辑文件）。
- **RED/GREEN：** 先复现 CLI 例外可绕过策略；后改为调用正式策略接缝与结构化拒绝原因。
- **验收与验证：** CLI 预览与真实服务选择同一个 source/错误码；不存在测试外的强制启用逻辑。

### 4c. 协作迁移 RoutePlan、CLI 与调度绑定 — M（G1-02～G1-05）

- **依赖：** 3、G1-01；**问题：** task-17/18/19 路径仍可能在绑定时绕过矩阵。
...
backend/app/ops/{macro_incremental_common,fred_incremental_watermark}.py
...
对应的 CLI / 宏观任务 / 调度绑定测试
task/task-01-source-registry/{task_plan,findings,progress,completion-check-audit}.md
...
**当前阶段：** Phase 0 设计推广已完成；G1-01（正式入口与受控配置核实）待执行。

**下一动作：** 并行完成 G1-01 的只读入口/配置核实；对工作包 1、2 的目标符号分别做 GitNexus
impact，记录基线，然后为 Capability 契约和 `macro_supplementary` 写 RED 测试。4c～4e 必须依
G1-02～G1-07 的阻塞边执行，不能提前宣称跨模块完成。
