...
| 5 | **L5R-CLOSE** | `VR-L5-001` + `VR-MODEL-001` | **独立**关账：per-VR 证据 bundle → `registry_proposed_delta.yaml`；**不**声称 production-ready | `research/registry_proposed_delta.yaml` · 矩阵 §1 | registry 三件套 commit | 双 mandatory pytest 绿（见下节） | `execute-evidence/b03-l5-close.txt` · `b03-model-close.txt` |
...
| `tests/test_layer5_evidence_chain.py`                                           | 强制 pytest     | L5-02 / L5R-CLOSE | 7 tests 绿                  | 低                    |
| `tests/test_migration_coverage.py`                                              | 强制 pytest     | MODEL-02 / L5R-CLOSE | 6 tests 绿（Execute 已建） | 低                    |
| `research/model-table-matrix.md`                                                | MODEL-01 SSOT   | MODEL-01          | WAVE0 §2 交付物             | 低                    |
...
| `/to-issues` 等价                         | ✅ `research/vertical-slices.md` + Vertical slices 表 | WAVE0 §2 L5R-BOOT..CLOSE 冻结                                                      | 无                                  |

**Plan 质检结论：** `PASS_FOR_DISPATCH`（2026-06-28：WAVE0 §2 竖条已对齐；`test_migration_coverage.py` 已在 worktree 存在，MODEL-02 TDD 切片保留 RED→GREEN 证据路径）
