...
| RawStore/FileRegistry/fetch_log                              | `R3_STAGED_FIXTURE_CLOSED`  | raw evidence/file registry/fetch log paths and tests                                           | Next datasource batch must bind mature provider ports to these primitives.                                      |
| DataSourceService / SourceRoutePlanner / capability registry | `R3_STAGED_FIXTURE_CLOSED`  | service facade, route planner, source registry/capability tests                                | Next datasource batch must complete at least one QMD-owned provider path, not another facade.                   |
| Vendor adapters / provider fetch ports                       | `R1_SCAFFOLD`               | adapter skeletons and disabled/probe ports                                                     | R3F-R/R3G must move selected candidates to bounded QMD-owned provider ports or explicitly keep disabled.        |
...
| Layer1 axes                                                  | `R3_STAGED_FIXTURE_CLOSED`  | YAML loader, spec validation, ingestion/evidence paths                                         | Next Layer1 batch must close production source binding or explicitly narrow Layer1 production promise.          |
| Layer2 sensors                                               | `R3_STAGED_FIXTURE_CLOSED`  | staged fixture registry, snapshots, WriteManager-gated writes                                  | Next Layer2 batch must move from staged fixture to declared live/sandbox source closure or narrow scope.        |
| Layer3 chains                                                | `R3_STAGED_FIXTURE_CLOSED`  | staged bundle loader/snapshot builder                                                          | Next Layer3 batch must complete the first real source/evidence path or narrow chain scope.                      |
| Layer4 markets                                               | `R3_STAGED_FIXTURE_CLOSED`  | CN_A staged market structure fixture                                                           | Next Layer4 batch must complete first real market source path or narrow Layer4 scope.                           |
| Layer5 evidence                                              | `R2_MINIMAL_VERTICAL_SLICE` | foundation validator and minimal staged evidence-chain pieces                                  | Next Layer5 batch must close full promised evidence-chain/instrument path or narrow scope.                      |
