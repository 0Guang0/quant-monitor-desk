"""M-DATA-03 AC-7 — tier_c_live_acceptance --report tests."""
...
EVIDENCE_CONTRACT = TEST_PROJECT_ROOT / "specs/contracts/live_tier_c_evidence_v1.yaml"
...
def _mock_outcome(source_id: str) -> LiveValidationOutcome:
    ...
    return LiveValidationOutcome(
        ...
        fetch_provenance=str(binding.get("fetch_provenance", "product_live_gated")),
    )
...
def _patch_mock_validation(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "backend.app.ops.tier_c_live_acceptance.run_tier_c_live_validation",
        _mock_validation,
    )
    monkeypatch.setattr(
...
def test_reportRun_writesThreeManifests(
    isolated_tier_c_data_root: Path,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """覆盖范围：一次 --report run 落盘 3 份 manifest
    测试对象：run_acceptance_report
    目的/目标：全量 report 为每 Tier C 源写出 live_tier_c_evidence_manifest.json
    验证点：3 个 manifest 文件；每行 evidence_manifest_path 指向存在文件
    ...
    report_path = tmp_path / "tier-c-report.json"
    ...
    for row in report["sources"]:
        ...
        manifest_path = isolated_tier_c_data_root / rel
    ...
    assert frozenset(p.parent.name for p in manifest_paths) == TIER_C_SOURCES
...
def test_acceptanceReport_hasContractTopLevelFields(
    ...
    目的/目标：report 工件符合 live_tier_c_evidence_v1 acceptance_report 段
    ...
    report_path = tmp_path / "tier-c-shape.json"
    run_acceptance_report(report_path, data_root=isolated_tier_c_data_root)
    report = json.loads(report_path.read_text(encoding="utf-8"))
    ...
    for row in report["sources"]:
        for field in per_source_fields:
            assert field in row
        assert row["fetch_provenance"] in {"product_live_gated", "mock_stub"}
...
def test_cliReportFlag_writesReportViaMain(
    ...
    """覆盖范围：scripts/tier_c_live_acceptance.py --report
    测试对象：tier_c_live_acceptance.main --report
    目的/目标：CLI 入口可触发 report 写出并 exit 0（mock validation）
    验证点：main 返回 0；report 文件存在且 sources 长度 3
    失败含义：运维/CI 无法通过 CLI 产出验收 JSON
    """
    from scripts.tier_c_live_acceptance import main
...
def test_reportRun_e2InspectNotApplicableForThreeSources(
    isolated_tier_c_data_root: Path,
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    """覆盖范围：--report 全量 3 源 E2 inspect 不变量
    测试对象：run_acceptance_report per-source e2_inspect_status
    目的/目标：Tier C validation_fetch 轨 E2 恒为 NOT_APPLICABLE
    验证点：3 行 e2_inspect_status == NOT_APPLICABLE；manifest acceptance 同步
    失败含义：Tier C 误跑 incremental E2 inspect
    """
    monkeypatch.setenv("QMD_ALLOW_LIVE_FETCH", "1")
    _patch_mock_validation(monkeypatch)
    _mock_f0_pass(monkeypatch)

    report_path = tmp_path / "tier-c-e2-report.json"
    exit_code = run_acceptance_report(
        report_path,
        data_root=isolated_tier_c_data_root,
    )
    ...
    for row in report["sources"]:
        ...
        manifest_path = isolated_tier_c_data_root / row["evidence_manifest_path"]
...
def test_reportRun_writesFailureArtifactOnFixableFail(
    ...
    目的/目标：FAIL_FIXABLE 时写出 tier_c_live_acceptance_failure_{run_id}.json
    ...
    report_path = tmp_path / "tier-c-fail-report.json"
    exit_code = run_acceptance_report(
        report_path,
        data_root=isolated_tier_c_data_root,
    )
    assert exit_code == 1
    artifacts = list(tmp_path.glob("tier_c_live_acceptance_failure_*.json"))
...
def test_reportRun_zeroRowsFails(
    ...
    验证点：kalshi 行 disposition==fail；failure_class==FAIL_FIXABLE
