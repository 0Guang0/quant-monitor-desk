...
from backend.app.datasources.capability_registry import OperationDisabledError
...
def _bind_live_data_root(data_root: Path) -> Path:
    from backend.app.ops.tier_c_live_acceptance import assert_isolated_live_data_root
...
def _default_instrument_id(source_id: str, binding: dict[str, Any]) -> str:
    ...
    if source_id == "web_search":
        return "fed rate cut outlook 2024"
...
def _create_validation_fetch_port(source_id: str, data_domain: str, binding: dict[str, Any]):
    if source_id == "web_search":
        from backend.app.datasources.fetch_ports.web_search_evidence_port import (
            create_web_search_evidence_fetch_port,
        )

        query = _default_instrument_id(source_id, binding)
        return create_web_search_evidence_fetch_port(queries=(query,), max_results=3)
    return create_product_live_fetch_port(source_id=source_id, data_domain=data_domain)
...
def run_tier_c_live_validation(source_id: str, data_root: Path) -> LiveValidationOutcome:
    """Product live fetch → raw evidence (validation_fetch, not incremental sync)."""
    from backend.app.ops.tier_c_live_acceptance import source_bindings
    ...
    port = _create_validation_fetch_port(source_id, data_domain, binding)
    req = FetchRequest(
    ...
    except OperationDisabledError as exc:
        return LiveValidationOutcome(
            source_id=source_id,
            fetch_status="DISABLED_SOURCE",
            row_count=0,
            detail=f"capability disabled: {exc}",
