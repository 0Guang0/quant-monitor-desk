...
| 来源 | 内容 |
|------|------|
| `B02_04` §5 | B02-SYNC-01..06 → §8 SYNC-01..06 |
...
| AC-SYNC-001 | crash pytest **或** handoff 文件 | S3–S4 → §9.5–9.6 |
...
|--------------|----------|------------|------------|-----|
| `tests/test_sync_orchestrator.py` | 契约 parity；reserved deferred；crash/recovery | 集合相等；DeferredJobTypeError；WRITING+write_id；recovery→COMPLETED | 断言失败；NIE；过早 COMPLETED | 9.1–9.3, 9.5–9.6 |
...
| 同上 | `test_syncJob_reservedDataQuality_returnsDeferredJobTypeError` | 同上 | S2 | 同上 |
| 同上 | `test_syncJob_reservedRevisionAudit_returnsDeferredJobTypeError` | 同上 | S2 | 同上 |
...
| 只关 matrix 不关 VR-SYNC-001 | §9.6 门控 |
| 隐藏 NIE | SYNC-03 专用异常 |
...
| 6 | SYNC-06 | recovery 测试 **或** handoff | 05 | 001 |
...
### 9.6 SYNC-06 — VR-SYNC-001 关闭门控

| 路径 A | `test_syncJob_incremental_crashWindow_*` + `recoverStuckWriting` GREEN → 关闭 VR-SYNC-001 |
|--------|-------------------------------------------------------------------------------------------|
| 路径 B | 写 `research/sync-001-handoff.md`（owner Round 3F · R3F-BR-03 · entrypoints · closure tests）→ VR-SYNC-001 re-defer |

---

## 10. Execute 交接 DoD

- [x] §9 证据齐 · §5.4+§6 · `validate-execute-handoff` 0 · VR-SYNC-002 关闭证据 · VR-SYNC-001 二选一（路径 A）
