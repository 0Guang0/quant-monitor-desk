...
| 019 plan/audit review       | `review/round3-019-plan-audit`            | after 019 plan exists; may run as preflight checklist    | Parallel review     | review agent         | Read-only review of 019 staged-only, source, lineage, double-counting, and test risks.         |
| 018C live manual probe plan | `debt/r3b275-018c-live-manual-probe-plan` | after disabled `tdx_pytdx` candidate evidence is visible | Parallel planning   | probe planning agent | Planning-only future TDX live/manual probe; no live fetch without explicit user authorization. |
...
`feature/round3-023a-evidence-foundation` may run beside `feature/round3-019-layer2-sensor` or `020` once `019` staged gate is closed and core-file ownership is explicit. It must remain a minimal foundation slice and must not implement full Layer5 bars/events/financials/valuation. `review/round3-019-plan-audit` is read-only and may run as soon as 019 planning artifacts exist. `debt/r3b275-018c-live-manual-probe-plan` is planning-only and may run once 018C disabled-candidate evidence is visible; it must not perform live fetch.
