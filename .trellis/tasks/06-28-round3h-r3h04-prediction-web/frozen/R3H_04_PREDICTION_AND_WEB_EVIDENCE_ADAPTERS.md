<!-- FROZEN: Plan protocol v4 · do not edit · source: docs/implementation_tasks/ROUND_3_REAL_DATA_PRODUCTION_ENTRY/BATCH_3H_REAL_DATA_PRODUCTION_ENTRY/R3H_04_PREDICTION_AND_WEB_EVIDENCE_ADAPTERS.md · frozen_at: 2026-06-28T07:13:59Z -->
...
### 9.0 boot_test_skeleton
| 已执行 | [x] |

### 9.1 probability_evidence_contract
| 已执行 | [x] |
...
### 9.4 web_search_evidence
| 已执行 | [x] |

### 9.5 registry_coordinator
| 已执行 | [x] |

### 9.6 clean_write_negative
| 已执行 | [x] |
...
**本地路径（gitignore；Execute 须 Read；architecture_only）：**

| 参考 | 阶梯 | QMD 目标 |
|------|------|----------|
| `参考项目/agents-for-openbb/30-.../main.py` | architecture_only | manual_review_staging 证据摘要字段 |
| `参考项目/agents-for-openbb/40-.../main.py` | architecture_only | 同上 |
| `coingecko_port.py`（本项目） | L2 模式 | kalshi/polymarket mock port 模板 |
| kalshi/polymarket API | L3 + live smoke | 用户 Gate：capped live |
| web_search | L3 mock | 用户 Gate：真实 API 延后 |

仅借鉴 bounded table/dashboard artifact 形状；禁止 agent loop、OpenAI client、openbb_ai runtime、fact 升格。
