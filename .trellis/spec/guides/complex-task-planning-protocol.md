...
> **Execute agent（v4.2）** — `EXECUTION_PLAN.md` + `frozen/*.md` + `EXECUTION_INDEX.md` + `implement.jsonl`  
> **Execute agent（v4.1 legacy）** — `research/00-EXECUTION-ENTRY.md` + 包内 `research/*`  
> **Audit 编排器（v4.2）** — `agents/audit-boot-v4.2.md` + `AUDIT.plan.md` + `EXECUTION_INDEX.md` §5 + `audit.jsonl`  
> **定位**：Trellis 管任务状态、hooks、验收门；**本协议管 Plan 阶段**流程、产出物、Skill 注册表。  
> **语言**：计划与验收用中文；代码标识符、命令、路径保持英文。  
> **活跃路径（2026-07-04）**：新复杂任务默认 **v4.2**（`EXECUTION_PLAN.md` + `EXECUTION_INDEX.md`）。v4.1 仅归档任务只读。`MASTER.plan.md` 仅 `tasks/archive/` 只读，或 `in_progress`/`planning` + 显式 `plan_protocol_version`/`manifest_protocol_version: "3"` 在途 shim；不得新建活跃 MASTER。
...
| **`plan.freeze.md`**            | Plan                   | 冻结自检（**§3.0v4.2** / v4.1 legacy · `templates/plan.freeze.md`） |
...
### 0.0.1 Plan 协议 v4.1（Execution Bundle · **legacy · 归档/在途只读**）
...
### 0.0.2 Plan 协议 v4.2（Slim Plan · 2026-07-04 起新复杂任务默认）

`task.json`：`meta.plan_protocol_version: "4.2"` · `meta.execute_entry: "EXECUTION_PLAN.md"`。

| 文档                                                  | 谁读                    | 放什么                                       |
| ----------------------------------------------------- | ----------------------- | -------------------------------------------- |
| **`EXECUTION_PLAN.md`**（任务根）                     | Plan + Execute **首读** | **唯一执行计划正文 SSOT**                    |
| **`EXECUTION_INDEX.md`**                              | Execute + Audit         | §1 步骤/证据 · §3 manifest · §5 Audit 追溯   |
| **`AUDIT.plan.md`**                                   | Audit                   | 维度验证矩阵 + Trace Authority               |
| **`implement.jsonl` / `audit.jsonl` / `check.jsonl`** | Execute / Audit hook    | 由 `generate-manifests` 从 INDEX §3 自动生成 |

**Plan 冻结工件（v4.2 Slim · 仅此六件套 + `task.json`）：** 上表三份 md + 三份 jsonl。**不要求** `research/*`、`plan.freeze.md`、`prd.md`、`frozen/`。

**Skill 过程稿（可选）：** GitNexus、澄清 session 等可写 `research/*` 或 ADR，但 **不** 作为 freeze 门禁工件；外部必读须摘录进 INDEX §3。

**v4.1 废止（新任务）：** 独立的 `00-EXECUTION-ENTRY` · `EXTERNAL-INDEX` · `plan-consolidation` · `to-issues-slices.md` — 职责并入 `EXECUTION_PLAN` + `EXECUTION_INDEX`。归档任务保持 v4.1 只读。

### 0.0 Plan 协议 v4（冻结三件套 · legacy）

`task.json` `meta.plan_protocol_version: "4"`（历史 shim；**新任务用 4.2**，见 `task_store.py`）。
...
> **定位：** `docs/implementation_tasks/` = Plan 输入；v4.2 冻结后正文在 Trellis 任务根 `EXECUTION_PLAN.md`，机器索引 `EXECUTION_INDEX.md`。
...
> **v4.2 活跃（2026-07-04 起）：** 仅 `EXECUTION_PLAN.md` + `EXECUTION_INDEX.md`；见 §0.0.2 · `trellis-execute` · `audit-boot-v4.2.md`。  
> **以下内容为 v4.1 legacy（归档/在途只读）。**
...
```text
Execute §11 交接 → Audit（7.pre→Boot v4.2→A1–A8→A9 主会话；v4.1 任务用 audit-boot-v4.1）→ Repair（若有 §4.3）→ **INDEX §2.1** + `uv run pytest -q` 复验 → Finish
```
...
Phase 5e 打包（v4.2：`EXECUTION_PLAN.md` + `EXECUTION_INDEX.md` §3；v4.1：ENTRY 包）
Phase 6  冻结薄三件套 + validate-plan-freeze
Phase 6  Execute（v4.2）：Boot 读 `EXECUTION_PLAN` + INDEX + implement.jsonl → INDEX §1 逐步 → handoff Audit
Phase 6  Execute（v4.1 legacy）：Boot 读 ENTRY + research + 路由表 → INDEX §1 逐步 → handoff Audit
Phase 7  Audit：7.pre GitNexus → **Boot v4.2**（v4.1 任务用 audit-boot-v4.1）→ A1–A8 → A9 → audit.report
...
| 步骤     | 执行者  | 内容                                                                                                                                                                                                                     |
| -------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Boot** | Execute | Read **`EXECUTION_PLAN.md`** + INDEX §1/§3 + implement.jsonl + **必做** skill（`reference.md` + `principles.md` + yaml）+ **条件** skill（`agent-toolchain` §Execute 细则）；v4.1 legacy：ENTRY + research 全文 + 路由表 |
...
| 步骤        | 执行者             | 内容                                                                                                                                    |
| ----------- | ------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
| **7.pre**   | **主会话**         | GitNexus/CodeGraph 刷新 → `research/gitnexus-audit-summary.md`；全文 Read **`agents/audit-boot-v4.2.md`**（v4.1：`audit-boot-v4.1.md`） |
...
| 7.1–7.8     | **A1–A8 子 agent** | Boot v4.2（v4.1：`audit-boot-v4.1.md`）+ 执行 **AUDIT.plan §1** 本维验证 + GitNexus/CodeGraph → `research/audit-a{n}-report.md`         |
...
- **Boot：** `agents/repair-boot-v4.2.md`（v4.1 legacy：`repair-boot-v4.1.md`）
...
| `EXECUTION_INDEX.md` §1/§3                        | Execute | 步骤 · manifest 路由（implement slot2） |
| `AUDIT.plan.md`                                   | Audit   | 维度矩阵（audit.jsonl 第一条）          |
| `implement.jsonl` / `audit.jsonl` / `check.jsonl` | hook    | 由 `generate-manifests` 生成            |

> **Boot 读序 SSOT：** `.cursor/skills/trellis-execute/SKILL.md` · Audit：`agents/audit-boot-v4.2.md`

### Execute 入口（v4.1 legacy）
...
| 序号 | v4.2 Slim                                 | v4.1 legacy                               | v3 legacy                                 |
| ---- | ----------------------------------------- | ----------------------------------------- | ----------------------------------------- |
| 1    | `EXECUTION_PLAN.md`                       | `frozen/*.md`（任务卡）                   | `MASTER.plan.md`                          |
| 2    | `EXECUTION_INDEX.md`                      | `research/00-EXECUTION-ENTRY.md`          | `context_pack.json`                       |
| 3    | `.cursor/skills/trellis-execute/SKILL.md` | `context_pack.json`                       | `.cursor/skills/trellis-execute/SKILL.md` |
| 4+   | INDEX §3 外部必读行                       | `.cursor/skills/trellis-execute/SKILL.md` | 任务接线 docs/specs/code                  |

> v4.2：**无** `MASTER.plan.md` · **无** ENTRY 包。`validate-plan-freeze` 在缺 pack 时自动调用 `context_router.py --task`。
...
> **一维一 agent。** §1 = Skill；**Boot = `agents/audit-boot-v4.2.md`**（v4.1：`audit-boot-v4.1.md`）；验证矩阵见 `audit-skill-registry.md` §2。
...
### 8.0 v4.2 Slim Plan（默认）

- [ ] `EXECUTION_PLAN.md` + `EXECUTION_INDEX.md`（§1/§3）+ `AUDIT.plan.md`
- [ ] `meta.plan_protocol_version: "4.2"` · `execute_entry: "EXECUTION_PLAN.md"`
- [ ] `task.py generate-manifests` → `implement.jsonl` · `audit.jsonl` · `check.jsonl`
- [ ] `validate-plan-freeze` exit 0（**不要求** `plan.freeze.md` / `research/*` / `frozen/`）

### 8.1 v4.1 legacy

> 逐项勾选：**`plan.freeze.md` 附录 A**（或归档任务 `plan.freeze.md` §3.0v4.1）
