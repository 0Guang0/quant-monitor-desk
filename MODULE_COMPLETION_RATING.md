...
> **Last reconciled:** **2026-07-02 Pass E** — 四路 agent 代码审计 + `master` @ `68f70206` · `uv run pytest -q` exit 0；prior Pass D @ `893e6e2b`（2026-07-01）。
...
| **mock/replay 天花板**     | C3 DCP-05 十一源 incremental e2e **全部 mock/replay**；Layer clean e2e 多为 **tmp DB seed**，仅 G5 mootdx 走 sync→clean 全链 |
...
| B2  | Data quality validator          | `validators`, `data_quality_rules.yaml`         | `R3_STAGED_FIXTURE_CLOSED`          | 2/3  | R3H-08 profiles · DCP-03 smoke      | R3→R5       | `validators/data_quality.py` · `tests/test_data_quality_validator.py` · `test_incremental_post_write_inspect.py`（**仅** baostock）                                                     | **M-DATA-03**                                               |
...
| C3  | Vendor adapters / provider fetch ports       | `datasources`, `fetch_ports/*`             | `R4_SANDBOX_REAL_DATA_OR_REHEARSAL` | 2/3  | R3H-08 port live gate · DCP-05 11源 replay incremental | R3→R4       | `fetch_ports/*` · `tests/test_r3h08_live_productization.py` · 11× `tests/test_*_incremental_e2e.py`（**全 mock/replay**） | **M-DATA-03** 11 源真网         |
...
| ID  | Module                         | Design authority                                 | Rating                              | 批/3    | Milestone                          | Close round | Evidence                                                                                                            | 活票 / 归属（§1.8）                                 |
| --- | ------------------------------ | ------------------------------------------------ | ----------------------------------- | ------- | ---------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------- |
| D1  | Data sync orchestration        | `sync_orchestrator`, `data_sync_orchestrator.md` | `R4_SANDBOX_REAL_DATA_OR_REHEARSAL` | **3/3** | DCP-01/02/05/09 · bounded backfill | R3→R5       | `sync/orchestrator.py` · `tests/test_sync_orchestrator.py` · 11源 incremental e2e · `test_qmd_data_backfill_cli.py` | **M-DATA-03** 真网验收                              |
...
| ID  | Module                                         | Design authority                  | Rating                                         | 批/3 | Milestone                      | Close round | Evidence                                                                                                                        | 活票 / 归属（§1.8）                 |
| --- | ---------------------------------------------- | --------------------------------- | ---------------------------------------------- | ---- | ------------------------------ | ----------- | ------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- |
| E1  | `qmd data` CLI                                 | `docs/ops/data_health_cli.md`     | `R4_SANDBOX_REAL_DATA_OR_REHEARSAL`            | 2/3  | DCP-01/02/05 router · backfill | R3→R5       | `cli/data_commands.py` · `tests/test_qmd_data_cli.py` · `test_qmd_data_sync_tier_a_router.py`（11 源 dry-run；**真跑仅 3 源**） | **M-DATA-03**                       |
| E2  | Ops DB inspect + verification matrix           | `docs/ops/db_inspect_cli.md`      | `R4_SANDBOX_REAL_DATA_OR_REHEARSAL` **（窄）** | 2/3  | DCP-03 baostock post-write     | R3→R5       | `ops/db_inspector.py` · `tests/test_incremental_post_write_inspect.py`（**仅 baostock**）                                       | **M-DATA-03**                       |
...
| ID  | Module             | Design authority           | Rating                                         | 批/3 | Milestone                   | Close round | Evidence                                                                                                      | 活票 / 归属（§1.8） |
| --- | ------------------ | -------------------------- | ---------------------------------------------- | ---- | --------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------- | ------------------- |
| F0  | Data health engine | `ops/data_health_profiles` | `R4_SANDBOX_REAL_DATA_OR_REHEARSAL` **（窄）** | 2/3  | DCP-03 `market_bar_p0` only | R3→R5       | `data_health_profiles/`（**仅** `market_bar_p0` × `market_bar_1d`）· `test_incremental_post_write_inspect.py` | **M-DATA-03**       |
