"""M-DATA-03 AC-7 — live_tier_b_evidence_v1 contract tests."""
...
EVIDENCE_CONTRACT = PROJECT_ROOT / "specs/contracts/live_tier_b_evidence_v1.yaml"
...
@pytest.mark.parametrize("source_id", sorted(TIER_B_SOURCES))
def test_buildManifest_requiredTopLevelFieldsPerSource(source_id: str) -> None:
    """覆盖范围：envelope.required_top_level_fields × 10 source_bindings
    测试对象：build_live_tier_b_evidence_manifest
    ...
    data_root = PROJECT_ROOT / ".audit-sandbox" / "m-data-03" / "tier-b" / "contract-fixture"
...
def test_buildManifest_fetchRequiredFieldsPerSource(source_id: str) -> None:
    """覆盖范围：envelope.fetch.required_fields × 10 source_bindings
    测试对象：build_live_tier_b_evidence_manifest fetch 段
    ...
    data_root = PROJECT_ROOT / ".audit-sandbox" / "m-data-03" / "tier-b" / "contract-fetch-fixture"
    manifest = build_live_tier_b_evidence_manifest(
        source_id=source_id,
        data_root=data_root,
        run_id="run-fetch",
...
def test_buildManifest_acceptanceMatchesSourceBinding(source_id: str) -> None:
    ...
    测试对象：build_live_tier_b_evidence_manifest acceptance 段
...
def test_writeManifest_persistsUnderEvidenceDir(tmp_path: Path) -> None:
    """覆盖范围：manifest 落盘路径
    测试对象：write_live_tier_b_evidence_manifest
    目的/目标：写入 live_tier_b_evidence_manifest.json 于 evidence_dir_relative
    ...
    manifest = build_live_tier_b_evidence_manifest(
        ...
        run_id="run-write",
        job_id="job-write",
        outcome=_sample_outcome("yahoo_finance"),
        report_failure_class="PASS",
    )
    path = write_live_tier_b_evidence_manifest(manifest, data_root=data_root)
    assert path.name == MANIFEST_FILENAME
    assert path.is_file()
    loaded = json.loads(path.read_text(encoding="utf-8"))
    assert loaded["source_id"] == "yahoo_finance"


def test_liveTierBEvidenceContract_fetchProvenanceOnEveryBinding() -> None:
    """覆盖范围：invariants.fetch_provenance_required_on_report
    测试对象：live_tier_b_evidence_v1.yaml source_bindings
    目的/目标：每源 binding 登记 fetch_provenance 与 default_instrument
    验证点：fetch_provenance ∈ {mock_replay, product_live_gated}；default_instrument 非空
    ...
    for source_id, binding in contract["source_bindings"].items():
        ...
        assert prov in {"mock_replay", "product_live_gated"}, source_id
        assert binding.get("default_instrument"), source_id


def test_liveTierBEvidenceContract_defaultInstrumentAlignsWithProductLiveDefaults() -> None:
    """覆盖范围：binding.default_instrument vs SOURCE_LIVE_DEFAULTS
    测试对象：live_tier_b_evidence_v1 source_bindings
    目的/目标：契约 default_instrument 与 product live 工厂首项一致
    验证点：每 Tier B 源 default_instrument 命中 SOURCE_LIVE_DEFAULTS 首 symbol/asset/concept
    失败含义：验收 dispatch 与产品 live 默认标的漂移
    """
    from backend.app.datasources.product_live_ports import SOURCE_LIVE_DEFAULTS

    contract = _load_contract()
    for source_id in sorted(TIER_B_SOURCES):
        binding = contract["source_bindings"][source_id]
...
def test_liveTierBEvidenceContract_validationOnlyInvariants() -> None:
    """覆盖范围：no_clean_upsert + e2_inspect_not_applicable
    测试对象：live_tier_b_evidence_v1 invariants
    目的/目标：Tier B 轨明确 validation_only 语义
    验证点：no_clean_upsert_for_validation_only；e2_inspect_not_applicable
    失败含义：Tier B 被误当作 incremental clean 轨
    """
    contract = _load_contract()
    merged: dict[str, Any] = {}
