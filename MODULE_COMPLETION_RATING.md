...
> **Last reconciled:** 2026-06-29（§3）；**2026-06-29b** 增补 `specs/` 全量 + `docs/`（排除 implementation_tasks 与导航/登记类 guide）交叉核对。
...
| ID  | Module                                   | Design authority                                                                                   | Rating                      | 批/3 | Close round | Evidence (2026-06-29)                                                                               | Required next movement                                                                    |
| --- | ---------------------------------------- | -------------------------------------------------------------------------------------------------- | --------------------------- | ---- | ----------- | --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
...
| A3  | Storage / evidence primitives            | `core_platform`, `docs/modules/local_file_system.md`                                               | `R3_STAGED_FIXTURE_CLOSED`  | 2/3  | R3→R5       | `backend/app/storage/**`, `test_raw_store.py`, `test_path_compat.py`, fetch_log/file_registry paths | R3H-08: every READY source binds real fetch evidence on live path; Batch05 lineage audit. |
| A4  | ResourceGuard / performance budget       | `core_platform`, `docs/ops/performance_limits.md`                                                  | `R3_STAGED_FIXTURE_CLOSED`  | 2/3  | R3→R5       | `backend/app/core/resource_guard.py`, sync/adapter guard tests, production-equivalent budget tests  | R3H-08 + Batch04/05: prove caps on every real adapter/API/review/sync smoke path.         |
...
| B2  | Data quality validator          | `validators`, `data_quality_rules.yaml`         | `R3_STAGED_FIXTURE_CLOSED`                  | 2/3  | R3→R5       | `validators/data_quality.py`, `test_data_quality_validator.py`, `market_bar_p0` profile                 | R3H-08/05: bind profiles to each ready source/domain before readiness claims.                           |
| B3  | Source conflict validator       | `validators`, `data_validation_and_conflict.md` | `R2_MINIMAL_VERTICAL_SLICE` → `R3` boundary | 2/3  | R3          | `source_conflict.py`, gate integration, severe-conflict tests                                           | R3H-08: conflict outcomes on live paths per ready source; manual_review_queue for unresolved reconcile. |
...
| C2  | DataSourceService facade                     | `datasources`, `datasource_service.md`                                 | `R4_SANDBOX_REAL_DATA_OR_REHEARSAL` | 2/3  | R3→R4       | `datasource_service.py`, `test_datasource_service.py`, CLI route-preview/sync-plan, ADR-025 fail-closed          | **R3H-10 CLOSED @ 2026-06-29:** contract active; STAGED-PILOT-SSOT closed; unlock R3H-07.              |
| C3  | Vendor adapters / provider fetch ports       | `datasources`, `data_sources.md`, `fetch_ports/*` (29 port modules)    | `R3_STAGED_FIXTURE_CLOSED`          | 2/3  | R3          | **R3H-01～04 CLOSED** @ 2026-06-28: 24 business sources `READY_WITH_EVIDENCE` or ADR-disabled; replay/mock ports | **Batch 3 = R3H-08:** env-gated live→Tier A/B/C product path (not only `--live-wire` / pilot scripts). |
...
| ID  | Module                         | Design authority                                             | Rating                      | 批/3 | Close round | Evidence                                                                                           | Required next movement                                                                                                |
| --- | ------------------------------ | ------------------------------------------------------------ | --------------------------- | ---- | ----------- | -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| D1  | Data sync orchestration        | `sync_orchestrator`, `data_sync_orchestrator.md`             | `R3_STAGED_FIXTURE_CLOSED`  | 2/3  | R3→R5       | `IncrementalJobRunner`, `BackfillShardRunner` (per-shard validate+write), orchestrator/job tests   | R3: watermark + R3H-08 product demo; Batch6: FullLoad/CLI/idempotency edge; Batch05: bounded production smoke.        |
...
| ID  | Module                                                       | Design authority                                                | Rating                              | 批/3 | Close round | Evidence                                                                                               | Required next movement                                                                    |
| --- | ------------------------------------------------------------ | --------------------------------------------------------------- | ----------------------------------- | ---- | ----------- | ------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| E1  | `qmd data` CLI (health / route / sync-plan / init / promote) | `ops`, `docs/ops/data_health_cli.md`                            | `R3_STAGED_FIXTURE_CLOSED`          | 2/3  | R3→R5       | `data_commands.py`: health @ `ecf64f06`, sandbox promote subcommands, `test_qmd_data_cli.py`           | R3: sync incremental CLI slice; Batch05 packaged operator distribution.                   |
| E2  | Ops DB inspect + verification matrix                         | `ops`, `docs/ops/db_inspect_cli.md`                             | `R3_STAGED_FIXTURE_CLOSED`          | 2/3  | R3→R5       | `db_inspector.py`, `test_ops_db_inspector.py`, `test_round3_verification_command_matrix.py`            | Tie inspect smoke to R3H-08 post-write checks (row counts, max trade_date).               |
...
| E4  | Live / staged pilot runners                                  | `ops`, `production_live_pilot_policy.md`                        | `R4_SANDBOX_REAL_DATA_OR_REHEARSAL` | 2/3  | R3          | `live_pilot_*`, `staged_pilot`, `test_batch275_live_pilot_gate.py`; pilot DB only                      | R3H-08 replaces pilot scripts with product path; pilot evidence remains audit input only. |
...
| ID  | Module             | Design authority                            | Rating                     | 批/3 | Close round | Evidence                                                                        | Required next movement                                                      |
| --- | ------------------ | ------------------------------------------- | -------------------------- | ---- | ----------- | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| F0  | Data health engine | `ops/data_health_profiles`, EasyXT profiles | `R3_STAGED_FIXTURE_CLOSED` | 2/3  | R3→R5       | `run_data_health_profile`, `market_bar_p0`, `cn_trading_calendar` profile tests | R3H-08 admission checks; Batch05 expands profiles only if scope ADR widens. |

### 3.G Modeling layers (Layer1–5)

| ID  | Module                     | Design authority                                   | Rating                      | 批/3 | Close round | Evidence                                                                            | Required next movement                                                                       |
| --- | -------------------------- | -------------------------------------------------- | --------------------------- | ---- | ----------- | ----------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- |
| G1  | Layer1 axes / regime panel | `layer1_axes`, `layer1_global_regime_panel.md`     | `R3_STAGED_FIXTURE_CLOSED`  | 2/3  | R3→R4+      | loader, 018A bridge PASS (staged), `test_layer1_*` (9 tests in authority_graph)     | R3: **one axis** clean→metric vertical slice; full G12 matrix = Round4+ (**not PASS gate**). |
| G2  | Layer2 cross-asset sensors | `layer2_sensors`, `layer2_cross_asset_sensor.md`   | `R3_STAGED_FIXTURE_CLOSED`  | 2/3  | R4+         | staged fixtures, snapshot writer, `test_layer2_sensor_loader.py`                    | Real market/validation source binding or ADR-narrow scope; after R3H-08.                     |
| G3  | Layer3 industry chains     | `layer3_chains`, `layer3_industry_shock_anchor.md` | `R3_STAGED_FIXTURE_CLOSED`  | 1/3  | R4+         | loader/snapshot builder; `test_layer3_loader.py`, `test_layer3_snapshot_builder.py` | Real CN chain evidence or ADR-narrow; registries **K3** in `authority_graph` v2.             |
| G4  | Layer4 market structure    | `layer4_markets`, `layer4_market_structure.md`     | `R3_STAGED_FIXTURE_CLOSED`  | 1/3  | R4+         | CN_A staged fixture; `test_layer4_market_structure.py`                              | US calendar (R3H-07) + real breadth/calendar sources or ADR-narrow.                          |
| G5  | Layer5 evidence / security | `layer5_evidence`, `layer5_security_evidence.md`   | `R2_MINIMAL_VERTICAL_SLICE` | 2/3  | R3          | foundation validator, evidence chain, web-evidence no-clean-write tests             | R3H-05: bind source_fetch_id/content_hash/schema_hash for production-entry scope.            |
| G6  | Manual review staging      | `backend/app/evidence/manual_review_staging.py`    | `R2_MINIMAL_VERTICAL_SLICE` | 1/3  | R3          | staging for kalshi/polymarket/web_search manual-review path                         | R3H-08D: live probability/manual-review bundle; no factual clean write.                      |
...
| K2  | Layer1 five-axis indicator specs  | `specs/layer1_axes/restructured_axes_v1_1/**`              | `R3_STAGED_FIXTURE_CLOSED` | 1/3  | R4+         | per-axis indicator_spec + engineering_rules (5 axes)                                | **Sub-scope of G1** — metric engine not built per axis.                 |
...
- **Round3 data-plane group** (recommended parallel closure before Round4 product work): **A3, B1–B3, C1–C4, D1, E1–E2, G1 (one axis), G5** — see `PROJECT_IMPLEMENTATION_ROADMAP.md` §5.0.6.
