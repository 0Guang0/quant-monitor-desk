...
> A5 另读 MASTER §2 / §8 / §10 / §11 与 Execute evidence。A1 另读 `check.jsonl` · Execute **§10 证据只读**（非复跑）
...
## 0.1 Trace Authority Set（复杂任务必填）

复杂任务 Audit 必须验证两件事：

1. Execute 是否按 MASTER 执行；
2. MASTER / AUDIT 是否完整继承原始任务、项目地图、轮次地图、任务输入索引和 unresolved/deferred registry。

Plan 必须填写下表，并将对应文件加入 `audit.jsonl`。

| 类别                       | 文件                                                         | 必读 agent | 用途                                                    |
| -------------------------- | ------------------------------------------------------------ | ---------- | ------------------------------------------------------- |
| 原始任务卡                 | `{{docs/implementation_tasks/.../NNN_task.md}}`              | A1, A5, A8 | 验证原始 scope / AC / Red Flags 是否进入 MASTER / AUDIT |
| implementation task README | `docs/implementation_tasks/README.md`                        | A1         | 验证 Plan 是否遵守原始任务入口                          |
| task input context index   | `docs/implementation_tasks/TASK_INPUT_CONTEXT_INDEX.md`      | A1, A5     | 验证必读上下文是否纳入                                  |
| unresolved coverage        | `docs/implementation_tasks/UNRESOLVED_ITEM_TASK_COVERAGE.md` | A1, A5, A8 | 验证待修复/未闭合项是否纳入 AC / §8 / Audit             |
| project map                | `MIGRATION_MAP.md`                                           | A1, A5     | 验证实现目录和 docs/specs 边界                          |
| round map                  | `{{ROUND*_BATCH_IMPLEMENTATION_MAP.md}}`                     | A1, A5     | 验证 batch item IDs / source bundle / out-of-scope      |
| original-plan-trace        | `research/original-plan-trace.md`                            | A1, A5     | 验证原始任务到 MASTER AC 的映射                         |
| input-inventory            | `research/input-inventory.md`                                | A1         | 验证六类关键信息覆盖                                    |
| project-map-omission-check | `research/project-map-omission-check.md`                     | A1, A5     | 验证项目地图倒查                                        |
| integration-ledger         | `research/integration-ledger.md`                             | A1, A5, A8 | 验证 context packing 决策                               |

如果任一 trace authority 缺失、未读或与 MASTER/AUDIT 存在未解释差异，A1 或 A5 必须列入 `audit.report.md` §4.3，不得 PASS。

**编排规则：**

1. **7.pre（主会话 · Execute 交接后）**：刷新 GitNexus / CodeGraph → `research/gitnexus-audit-summary.md`。
2. **7.pre.1（主会话 · A-lite）**：Trace Authority Presence Check — 仅确认 Trace Authority Set + `audit.jsonl` 入口完整；缺失则退回 Plan，不派发 A1–A8。
3. **7.0（主会话）**：汇总 **§2 各维验证证据** + Execute §10 证据索引（只读）→ `audit.report.md` §2。
4. **7.1–7.8**：按 §1 派发 **A1–A8**；各 agent 执行 **§2 本维冻结验证** + GitNexus/CodeGraph（§3）。
5. **7.9（主会话）**：A9 汇总 §4 / §4.3。
6. **PASS（无 §4.3）** → Phase 9；**PASS_WITH_FIXES / FAIL** → `REPAIR.plan.md` → Phase 8。
...
| ------ | --------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | -------------------------------------------------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------- | ------ | ------ |
| **A1** | read-only       | trellis-check；diff vs `check.jsonl`；**original-source trace**（Trace Authority Set）；对照 `docs/implementation_tasks/` 任务卡 ↔ MASTER §1.3/§2 AC；GitNexus query 未在 implement.jsonl 声明的依赖                              | local           | 无写                                                           | 无未授权 spec 偏离；无 Plan omission；无 ghost 依赖    | A1 **必须**对照 Trace Authority Set，判断 MASTER/AUDIT 是否遗漏或擅自扩缩原始 scope | §3.1   | [ ]    |
...
| **A5** | trace-ac        | **AC source-chain trace**：从原始任务卡、Round map、unresolved coverage、registry 追溯到 MASTER §2 AC、§8、Execute evidence、closeout/re-defer；证据完整性 1–5 分；若存在 `research/manifest-amend.md` 须验证 amend 已补读（E20） | local           | 无写                                                           | 每条 AC 可追溯；均 ≥4 分                               | A5 **必须**从原始来源验证 evidence 覆盖 source-chain，不得只检查文件存在            | §3.5   | [ ]    |
...
| **A8** | pytest-isolated | **original Red Flags test-gap trace**；对照 `018B`/policy/unresolved coverage；`pytest {{tests/audit/ 或 -k audit_}} --tmp-path {{}}` 补边界；**每测须满足 testing-guidelines**                                                   | audit-sandbox   | tmp DB / `@pytest.mark.audit`                                  | 新测全绿；原始 Red Flags 有测试或 explicit audit check | A8 **必须从原始任务 Red Flags 出发**，不得只看 MASTER §7 压缩路径                   | §3.8   | [ ]    |
