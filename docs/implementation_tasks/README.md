...
> **当前下一执行入口：** `ROUND_3_REAL_DATA_PRODUCTION_ENTRY/BATCH_3H_REAL_DATA_PRODUCTION_ENTRY/` — **`R3H-10` → `R3H-07` 串行**（Wave 1）；INDEX：`WAVE1_R3H10_THEN_R3H07_TO_ISSUES_INDEX.md`；总施工图：根目录 `PROJECT_IMPLEMENTATION_ROADMAP.md`（模块轨）；PASS 索引：`R3H_PASS_EXECUTION_PLAN.md`。
...
11. `ROUND_3_REAL_DATA_PRODUCTION_ENTRY/`：Round 3H；**R3H-01～04、R3H-06、Batch3V CLOSED**；**当前：PASS Wave 1** — **`R3H-10` CLOSED 后才 `R3H-07`**；Wave 2–5 见 `R3H_PASS_EXECUTION_PLAN.md` §3；模块轨见根目录 `PROJECT_IMPLEMENTATION_ROADMAP.md` §3；**五轴 G12 PASS 前全绿**。
...
- `PRODUCTION_COMPLETION_VERTICAL_SLICE_PLAN.md` 只是覆盖地图，用来检查哪些模块还没到完整生产级；它不是工单。真正执行时，仍以对应 batch 的具体任务卡为准。通俗说：**地图不是工单；任务卡才是工单。**
...
- `MIGRATION_MAP.md` — **全部任务**（Plan / Execute / Audit / Repair）共同必读；跨目录定位设计文档、契约、规则与实现目录入口
...
> **Forward planning:** `PROJECT_IMPLEMENTATION_ROADMAP.md`（根目录）为前向施工导航（**地图不是工单**；真正执行在任务卡）；`PRODUCTION_COMPLETION_VERTICAL_SLICE_PLAN.md` 仅为覆盖缺口对照；`ROUND3_BATCH_IMPLEMENTATION_MAP.md` 保留历史批次与证据索引。  
...
> **Status:** **CLOSED** (R3FR-07).  
> **Forward planning:** root `PROJECT_IMPLEMENTATION_ROADMAP.md` Round 3G.  
> **Historical entrypoint:** `ROUND_3_REFERENCE_ADOPTION_REFACTOR/BATCH_3FR_REFERENCE_ADOPTION_REFACTOR/README.md`  
...
> **Batch status:** **CLOSED** @ R3G-03（2026-06-27）。勿再开 R3G-04 Trellis；运维预演见 `R3G_MASS_REHEARSAL_OPEN_GAPS.md` 与 `scripts/r3g03_isolated_pilot_dry_run.py --live-wire`。  
> **Preconditions:** Round 3F-R **CLOSED** (condition A).  
...
- `ROUND_3_SANDBOX_CLEAN_WRITE/BATCH_3G_SANDBOX_CLEAN_WRITE/README.md` — Round 3G canonical entrypoint（归档）
...
- `ROUND_3_SANDBOX_CLEAN_WRITE/BATCH_3G_SANDBOX_CLEAN_WRITE/R3G_MASS_REHEARSAL_OPEN_GAPS.md` — 3G 后预演暴露项 → 3H 归属（G1–G17）

## ROUND_3_REAL_DATA_PRODUCTION_ENTRY

> **Current executable entrypoint:** `ROUND_3_REAL_DATA_PRODUCTION_ENTRY/BATCH_3H_REAL_DATA_PRODUCTION_ENTRY/README.md`  
> **Wave 1 entry (serial):** `R3H_10_DATASOURCE_SERVICE_SSOT.md` → then `R3H_07_US_TRADING_CALENDAR.md`  
> **Wave 1 `/to-issues`:** `WAVE1_R3H10_THEN_R3H07_TO_ISSUES_INDEX.md`  
> **Gate:** Round4 blocked until `R3H-05-GATE` = **`PASS_ROUND4_REAL_DATA_READY`** (incl. G12 five-axis green).
...
- `ROUND_3_REAL_DATA_PRODUCTION_ENTRY/BATCH_3H_REAL_DATA_PRODUCTION_ENTRY/WAVE1_R3H10_THEN_R3H07_TO_ISSUES_INDEX.md` — Wave 1 serial `/to-issues` skeleton
- `ROUND_3_REAL_DATA_PRODUCTION_ENTRY/BATCH_3H_REAL_DATA_PRODUCTION_ENTRY/R3H_10_DATASOURCE_SERVICE_SSOT.md` — **current** Wave 1a entry (C2 SSOT)
- `ROUND_3_REAL_DATA_PRODUCTION_ENTRY/BATCH_3H_REAL_DATA_PRODUCTION_ENTRY/R3H_07_US_TRADING_CALENDAR.md` — Wave 1b (blocked by R3H-10)
- `ROUND_3_REAL_DATA_PRODUCTION_ENTRY/BATCH_3H_REAL_DATA_PRODUCTION_ENTRY/R3H_PASS_EXECUTION_PLAN.md` — PASS wave index (module track)
- `ROUND_3_REAL_DATA_PRODUCTION_ENTRY/BATCH_3H_REAL_DATA_PRODUCTION_ENTRY/R3H_05_LAYER_BINDING_AND_PRODUCTION_ENTRY_AUDIT.md` — Wave 5 audit GATE only (not current entry)
