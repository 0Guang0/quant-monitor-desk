...
Wave 0（本文 Batch 3V CLOSED）
  → [历史] R3H-06 Clean Schema（✅ CLOSED）
  → Wave 1 R3H-10 → R3H-07（串行）          ← 当前活轨
  → Wave 2 R3H-08A..D
  → Wave 3–4 R3-DCP（增量 + 五轴全绿）
  → Wave 5 R3H-05 审计 → PASS
  → Round 4（B04-01 先）
```

**Wave 0 明确不做：** live fetch、production clean write、`web_search` 真 API（见 `R3H_PASS_EXECUTION_PLAN.md` §1）。

**下游（2026-06-29 模块轨）：** 见 `WAVE1_R3H10_THEN_R3H07_TO_ISSUES_INDEX.md`。
