...
|-------|--------|---------|------|--------|--------|
| trellis-execute | 必做 | 8.0 | 每步 | Read SKILL.md Phase 0 Boot | [ ] |
| test-driven-development | 必做 | 8.1–8.9 | 每步写代码 | RED→GREEN per §8.x | [ ] |
| incremental-implementation | 必做 | 8.1–8.11 | 全程 | 一次只完成当前 §8.x | [ ] |
| systematic-debugging | 条件 | 当前失败步 | pytest RED | 根因→最小修复 | [ ] |
| karpathy-guidelines | 必做 | 8.x 实现前 | 每步 GREEN 前 | User Rule | [ ] |
| testing-guidelines | 必做 | 8.x 测试 | 每步 | User Rule | [ ] |
| GitNexus impact | 必做 | 改 symbol 前 | 非平凡编辑 | `impact({target, upstream})` | [ ] |
