...
> **状态：** Wave 3 **CLOSED** @ 2026-06-30（DCP-01/02/03 全闭合）· 下一入口 **Wave 4** `R3-DCP-05..10`
...
**Wave 3 Done：** DCP-01/02/03 全闭合；下一协调入口 **Wave 4** `R3-DCP-05..10`。

---

## 6. Wave 4 开放项（live 验收承接 · 2026-07-01）

> **状态：** 🔴 OPEN · 见 `PROJECT_IMPLEMENTATION_ROADMAP.md` §3.5.2 · `待修复清单.md` §4 / §8  
...
### 6.1 R3-DCP-05 · Tier A 真网增量扩展

**承接台账：** `ACC-BAOSTOCK-NO-LIVE` · `ACC-EASTMONEY-TAXONOMY-001`

- [ ] `data_commands.sync_baostock_incremental`：`QMD_ALLOW_LIVE_FETCH=1` 时 `use_mock=False`（产品 live port）
- [ ] 隔离库 `qmd data sync --no-dry-run` baostock 真网 smoke（非 replay mock）
- [ ] 其余 Tier A 主源 watermark 增量（复制 DCP-01/02 模式）
- [ ] CN validation 口径：产品 `eastmoney_port` mock vs akshare `stock_zh_a_hist` 真网 — 文档/registry SSOT（**不关** `R3-B2.75-REQ2-EM`）

### 6.2 R3-DCP-06 · 五轴全绿 + L1–5 E2E live

**承接台账：** `ACC-LAYER-E2E-LIVE-001`

- [ ] §3.5.1 五轴清单 **全部 [x]**
- [ ] 真网 → clean → L3 快照 → L4 结构 → L5 证据 **非** `staged_fixture_only` 端到端 smoke
- [ ] 下游 Wave 5 `R3H-05-GATE` 审计门
