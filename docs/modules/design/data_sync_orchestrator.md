...
DataSyncOrchestrator 是后台任务总控，不负责具体抓取实现，也不直接写可信最终库或连续监控区；所有写入均交给 WriteManager。
...
| `WRITING`                | 正在由 WriteManager 写可信最终库、连续监控区、snapshot 或审计归档 |
| `COMPLETED`              | 完成                                                              |
...
| source conflict severe | 创建 ReconcileJob，不写可信最终库；仅满足连续监控准入时保留带人工复核标签的版本 |
| WriteManager rollback  | job 标记失败但可信最终库和连续监控区均无半写入                                  |
| 断点续跑               | 已完成 task 不重复写                                                            |
...
调度器只执行持久化 RoutePlan，不得自定义 source/platform 放行。Primary 失败时按领域固定候选顺序
执行合格 fallback；每次尝试、失败原因和连续监控写入必须可审计。代码、适配器、格式或 schema
失败同时创建高优先级修复事件。主源恢复后，调度器按该领域更新频率和前后缓冲窗口创建回补任务，
并将恢复 RoutePlan 交由 WriteManager 完成验证、归档与默认读取切换。
