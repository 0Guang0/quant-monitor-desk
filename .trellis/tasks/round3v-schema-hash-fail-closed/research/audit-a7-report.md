...
| 分支 | `fix/round3v-schema-hash-fail-closed`（未提交 diff） |
| 模式 | **只读 Audit**（不改代码、不 commit、不跑 init_db 写盘） |
...
| 项 | 冻结要求 | 审计结论 |
|----|----------|----------|
| migration | 无改动 | **PASS** — diff 未触及 `backend/app/db/migrations/`、`scripts/init_db.py`、`specs/schema/schema.sql` |
| registry | 无 registry 文件改动 | **PASS** — 未触及 `specs/datasource_registry/source_registry.yaml`、`source_capabilities.yaml`、`UNRESOLVED_*` registry 三件套 |
| 通过条件 | 零 schema/registry diff | **PASS** — 见 §2 |

**本任务 A7 主判据为静态边界**（零 schema/registry diff），与 MASTER「无 migration」一致；标准 A7 kill-migrate  walkthrough 因无 migration 面 **不适用**，以 gate fail-closed 异常场景替代（§3.7）。

---

## 2. Diff 范围核对（schema / registry / 数据面）

### 2.1 变更文件清单（`git diff --name-only`，6 项）

| 文件 | 类别 | A7 相关 |
|------|------|---------|
| `backend/app/db/validation_gate.py` | 应用逻辑（只读 SQL） | 读 `fetch_log` / `file_registry` / `validation_report`；无 DDL/DML |
| `backend/app/datasources/adapters/skeleton_base.py` | Adapter 推导 | 有界 CSV/Parquet schema_hash；临时 DuckDB 内存连接 |
...
### 2.2 显式未触及路径（对抗性 grep + diff）

- `specs/schema/schema.sql`
- `backend/app/db/migrations/**`
- `scripts/init_db.py`
- `specs/datasource_registry/source_registry.yaml`
- `docs/implementation_tasks/**/UNRESOLVED*.md` / `AUDIT_DEFERRED_REGISTRY*`
...
**DOUBT（DBA）：** 第二次 init 是否仅「不报错」？→ 本 diff **无 migration**，schema 不变；回归依赖 `test_schema_migration.py`（10 passed，见 §3.7）。
...
| 步骤 | 命令 | exit | 关键输出 / 证据 |
|------|------|------|-----------------|
| schema/registry diff 静态检查 | `git diff --name-only` | 0 | 6 文件；无 `specs/schema`、无 `migrations`、无 `datasource_registry` |
| migration 回归（只读 pytest） | `uv run pytest tests/test_schema_migration.py -q --basetemp=.audit-sandbox/pytest-mig` | **0** | `..........` 10 passed |
...
| 场景 | 命令 | exit | 日志 / 证据 |
|------|------|------|-------------|
| Gate 缺 schema_hash 拒绝 | `uv run pytest tests/test_db_validation_gate.py::test_missingSchemaHashOnStructuredFetch_rejects -q` | **0** | `ValidationRejected` + `schema_hash` |
| Gate 全量回归 | `uv run pytest tests/test_db_validation_gate.py -q --basetemp=.audit-sandbox/pytest-gate` | **0** | 13 passed |
| schema_hash 漂移拒绝 | `test_schemaHashDriftWithoutApproval_rejects`（同上文件） | 含于 13 passed | 既有负向未削弱 |
| A8 冻结子集（交叉） | `uv run pytest tests/test_data_adapter_contract.py tests/test_db_validation_gate.py tests/test_adapter_skeletons.py -q` | **1** | 103 passed / **1 failed** — 见 §4 F2 |
...
| Execute evidence 可 RCA | `9.0-green.txt`（126 passed）、`9.1`–`9.4` 分步 green 含真实 pytest 输出，非占位符 |
...
| F2 | A8 子集跑时 `test_skeletonAdapterBase_successWritesRawFile` 失败（raw 路径文件不存在） | NON-BLOCKING（A7） | 疑为 `QMD_DATA_ROOT` / basetemp 路径与 RawStore 落盘不一致的环境问题；**非** migration/registry 面；建议 A8 独立复现 |
| F3 | Parquet schema 推导每次 fetch 创建临时文件 + 内存 DuckDB | NON-BLOCKING | 有 `_CSV_SCHEMA_PREFIX_BYTES` 有界；temp 文件 `finally` 清理；高并发下 temp 目录 I/O 需容量关注（与 A6 交界，本任务 A6 SKIP） |
| F4 | Gate 结构化 fallback 查 `file_registry` 最近 `json/csv/parquet` | NON-BLOCKING | `LIMIT 1`；若 `raw_file_paths` 无后缀且 registry 历史行误导，可能过度判定 structured — 契约层应保证 path/registry 一致 |
...
# gate fail-closed
uv run pytest tests/test_db_validation_gate.py -q --basetemp=.audit-sandbox/pytest-gate
