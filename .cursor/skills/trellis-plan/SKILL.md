...
| **P0L** | **`context_router`（freeze 时自动；可选手动 `uv run python scripts/context_router.py --task <dir>`）** | `context_pack.json` + `research/context-router-output.md` + 证据 JSON 初稿 | freeze            |
...
3. `implement.jsonl` 第一条 = MASTER；loop 启用时第二条 = `context_pack.json`，第三条 = `trellis-execute/SKILL.md`（见 §6.1.1 / protocol）
4. `uv run python scripts/check_docs_specs_indexed.py`（机器遗漏检查；`MIGRATION_MAP.md` 为人类全量导航，非 freeze 硬门禁）
5. `python .trellis/scripts/task.py validate-plan-freeze <task-dir>` → exit 0
6. 用户批准 → `task.py start`
