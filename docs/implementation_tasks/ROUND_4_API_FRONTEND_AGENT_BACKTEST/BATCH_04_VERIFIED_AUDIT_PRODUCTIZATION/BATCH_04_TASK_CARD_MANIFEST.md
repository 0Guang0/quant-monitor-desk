...
> **Round4 gate:** Batch04 cannot start until R3H-05-GATE returns **`PASS_ROUND4_REAL_DATA_READY`** or **`WARN_ROUND4_ALLOWED_WITH_NARROWED_SCOPE_ADR`** (see `PROJECT_IMPLEMENTATION_ROADMAP.md` §5.0.5). **`web_search` live API** is the only deferred scope ADR (post-Round4 module `R3H-WEB-SEARCH`). **`BLOCK_ROUND4_DATA_ENTRY_INCOMPLETE`** blocks Round4; `BLOCK_*` blocks Round4.  
