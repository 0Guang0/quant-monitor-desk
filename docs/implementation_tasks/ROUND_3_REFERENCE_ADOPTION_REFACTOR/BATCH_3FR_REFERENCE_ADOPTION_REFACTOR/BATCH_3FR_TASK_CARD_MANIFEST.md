...
| `R3FR-03` | `R3FR_03_TDX_PROVIDER_REFACTOR.md`                | TDX moves from ad hoc probe to QMD-owned disabled/raw-only provider port                           | `backend/app/datasources/fetch_ports/tdx_pytdx_port.py`, `normalizers/tdx.py`, probe/registry tests                                       | Complete disabled/raw-only provider shape                             |
...
| `R3FR-05` | `R3FR_05_PROVIDER_CATALOG_OPENBB_REFERENCE.md`    | **Done** — provider catalog SSOT (25 sources) + OpenBB architecture-only reference on `feature/round3fr-provider-catalog` | `provider_catalog.yaml`, provider catalog tests, source contracts, schema CHECK enum alignment                                            | Catalog fields and all registry entries in one batch                  |
