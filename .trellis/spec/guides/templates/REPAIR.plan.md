...
> **产出：** 更新 `audit.report.md` §5 复验；必要时补丁 `EXECUTION_INDEX.md` §1/§2.1
...
| verification-before-completion | 必做      | 收尾         | 复跑 **INDEX §2.1** Tier + `uv run pytest -q` | [ ]    |
...
- [ ] §1 全部可修项 **已修复 + 证据**
- [ ] §3 Deferred 已用户确认（若有）
- [ ] **复跑 INDEX §2.1** Tier A/B/C 全绿 + `uv run pytest -q` exit 0（Execute 回归门禁；**非** AUDIT §1 全矩阵）
