...
本目录包含 017–023 共 7 个正式 implementation task 文件，并新增 `018A_layer1_observation_ingestion_bridge.md` 作为 Batch 2.5 强制桥接执行文件、`018B_production_live_pilot_gate.md` 作为 Batch 2.75 受控生产/live 数据试点门禁文件、`018C_tdx_pytdx_low_cost_probe.md` 作为 Batch 2.75 后续低成本数据接口候选源 sidecar probe 任务。必须按 `017` → `018` → `018A` → `018B` → `018C`（仅在 Batch 2.75 Request 2 evidence reconciliation 后）→ `019` 顺序进入真实数据相关下游工作。
