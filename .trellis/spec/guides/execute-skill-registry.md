...
incremental-implementation（跨文件时）
systematic-debugging（条件）
...
|-------|----------|------------------|
| Superpowers **test-driven-development** | §8 每步写代码 | 复杂任务 → **必做** |
| addy **incremental-implementation** | §8 多文件切片 | 跨文件 → **必做**；单文件 → **不用** |
| addy **source-driven-development** | §8 指定步 | 框架 API 步 → **条件**（写死 §8.x + 文档 URL） |
| Superpowers **systematic-debugging** | 当前失败 §8 步 | **条件**（触发=`pytest` RED） |
...
TDD 管红绿；incremental 管**垂直切片**。Plan 在 §8 分 8.1/8.2…；§12 标 **必做**，绑定「§8 全程」。
