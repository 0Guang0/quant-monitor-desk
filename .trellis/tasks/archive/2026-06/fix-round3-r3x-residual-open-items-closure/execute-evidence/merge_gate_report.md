...
| **Evidence index (α-3 backfill)** | `closed_claim_evidence_index.yaml` |
...
| ADV-R3X-DOC-001 | **FIXED** | `adversarial_audit_report.md` 刷新 |
...
## Execute evidence（α-3 backfill @ `fix/r3y-prompt15-evidence`）

| 文件 | 映射 |
| ---- | ---- |
| `closed_claim_evidence_index.yaml` | 73 claim_id → green.txt + pytest 交叉引用 SSOT |
| `g1-adv-r3x-green.txt` | §4.1 ADV-R3X（20 IDs） |
| `g2-adv-a1-green.txt` | §4.2 ADV-A1（15 IDs） |
| `g3-adv-a2-green.txt` | §4.3 ADV-A2（12 IDs；R3/R4 经 merged_claim_aliases 映射） |
| `g4-adv-a3-green.txt` | §4.4 ADV-A3（16 IDs） |
| `g5-adv-a5-a6-green.txt` | §4.5 ADV-A5/A6（5 IDs；含 R4→A6-003 合并项） |
| `g6-f019-green.txt` | §4.6 F-019（3 IDs） |
| `g7-registry-green.txt` | §4.7 Registry（R1/R2 + ADV-R3X-DOC-001；R3→DOC alias） |
| `umbrella-r3x-green.txt` | 18-case 伞测回归 |
| `full-pytest-green.txt` | Tier B 全量 pytest |

审计门禁：`tests/test_r3x_residual_open_items_closure.py::test_r3yPrompt15_closedClaimEvidenceIndexMapsToGreenTxt`

## Tests

```bash
uv run pytest tests/test_r3x_residual_open_items_closure.py -q
# 19 passed (18 umbrella + evidence index gate)

uv run pytest -q
# 966 passed, 4 skipped — see full-pytest-green.txt
```
...
**READY** — Master Checklist OPEN **0**；execute `*-green.txt` 映射经 α-3 补链；`pytest -q` 全绿后可 merge（协调者审查）。
