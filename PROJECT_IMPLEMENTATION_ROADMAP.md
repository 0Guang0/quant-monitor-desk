...
> **Status checkpoint:** `master` @ `584081af`（post Wave C registry reconcile + repo hygiene）；Wave A/B/C 已归档，`019`/`020`/`021`/`022`/`023A`、staged pilot v2、read-only data health v1、R3Y 关键修复已进入主线。  
...
## Batch 3D.1 — Full Layer5 Evidence Chain（串行主线）

**建议分支：** `feature/round3-023b-evidence-chain-full`  
...
## Batch 3D.2 — Five-Layer Model Input Whitelist（可与 3D.1 早期并行）

**建议分支：** `chore/round3-model-input-whitelist`  
...
## Batch 3D.3 — Lineage & Layer3 Hygiene Debt（可并行）

**建议分支：** `fix/round3-batch6-lineage-and-layer3-hygiene`  
...
## Batch 3E.1 — FRED-only Authorized Sandbox Pilot

**建议分支：** `feature/round3-fred-authorized-sandbox-pilot`  
...
## Batch 3E.2 — TDX Manual Probe & Validation-only Candidate

**建议分支：** `debt/round3-tdx-manual-probe`  
...
## Batch 3E.3 — baostock / cninfo / akshare Model-driven Expansion v3

**建议分支：** `feature/round3-real-data-staged-pilot-v3`  
...
## Batch 3E.4 — Read-only Data Health v2

**建议分支：** `feature/round3-readonly-data-health-v2`  
...
1. **Round 3D.1**：full `023` Layer5 evidence chain。
2. **Round 3D.2 / 3D.3**：模型输入白名单 + lineage/Layer3 hygiene，可并行。
3. **Round 3E.1 / 3E.2 / 3E.3**：FRED-only pilot、TDX manual probe、baostock/cninfo/akshare v3，可并行但合并前统一 source readiness。
4. **Round 3E.4**：data health v2。
5. **Round 3F**：Batch6 data governance / migration / CLI / source health / backfill / hygiene。
