...
| B2.5-O-05 | Live FRED `primary_source` for `ENV-E1-DGS10` vs staged `macro_supplementary`  | **RE-DEFERRED — Batch 6** FRED-only authorized live pilot | `018B_production_live_pilot_gate.md` · MASTER AC-P2-0 | No              | **Not closed by Batch 2.75 Request 3.** Staged route + Request 3 supplementary shape green; live FRED requires separate user-authorized `FRED:DGS10` pilot with sandbox/no-mutation proof (`pytest tests/test_fred_staged_semantics.py`) |
