"""Tier B live acceptance harness (M-DATA-03 AC-7 · validation_fetch)."""
...
TIER_B_SANDBOX_SEGMENT = "tier-b"
DEFAULT_SANDBOX_ROOT = (
    PROJECT_ROOT / ".audit-sandbox" / M_DATA_03_SANDBOX_SEGMENT / TIER_B_SANDBOX_SEGMENT
)

QUICK_SOURCE_IDS: tuple[str, ...] = ("yahoo_finance", "akshare")

SOURCE_API_KEY_ENV: dict[str, str] = {
    "ths_ifind": "THS_IFIND_LICENSE_ARTIFACT",
    "qmt_xtdata": "QMT_XTDATA_AUTHORIZED",
    "qmt_xqshare": "QMT_XQSHARE_AUTHORIZED",
}

EVIDENCE_CONTRACT_PATH = PROJECT_ROOT / "specs/contracts/live_tier_b_evidence_v1.yaml"
MANIFEST_FILENAME = "live_tier_b_evidence_manifest.json"
SCHEMA_VERSION = "live_tier_b_evidence_v1"
...
def fail_external_adr_ref() -> str:
    """ADR slug for FAIL_EXTERNAL rows — SSOT: live_tier_b_evidence_v1 authoritative_docs."""
    for doc in _load_evidence_contract().get("authoritative_docs") or []:
        match = re.search(r"ADR-\d+", str(doc))
        if match:
            return match.group(0)
    raise RuntimeError("live_tier_b_evidence_v1: no ADR in authoritative_docs")


class TierBLiveEnvError(RuntimeError):
...
def assert_isolated_live_data_root(data_root: Path | str) -> Path:
    """Reject canonical main DB; require `.audit-sandbox/m-data-03/tier-b`."""
    ...
    if (
        ...
        or TIER_B_SANDBOX_SEGMENT not in posix
    ):
        raise TierBLiveEnvError(
            f"DATA_ROOT must be under .audit-sandbox/{M_DATA_03_SANDBOX_SEGMENT}/{TIER_B_SANDBOX_SEGMENT}: {resolved}",
...
def missing_api_keys_for_sources(source_ids: tuple[str, ...]) -> list[str]:
    ...
    for sid in source_ids:
        ...
        if sid == "qmt_xqshare":
            host = os.environ.get("XQSHARE_REMOTE_HOST", "").strip()
            port = os.environ.get("XQSHARE_REMOTE_PORT", "").strip()
            if not str(raw).strip() or not host or not port:
                missing.append(
                    f"{env_name}, XQSHARE_REMOTE_HOST, XQSHARE_REMOTE_PORT (required for {sid})"
                )
            continue
...
def validate_live_acceptance_env(
    *,
    ...
    """Validate env for Tier B validation_fetch acceptance."""
    ...
    targets = source_ids or tuple(sorted(TIER_B_SOURCES))
...
def select_source_ids(
    *,
    ...
    if source_id is not None:
        if source_id not in TIER_B_SOURCES:
            raise TierBLiveEnvError(f"unknown Tier B source_id: {source_id!r}")
        return (source_id,)
    if quick:
        return QUICK_SOURCE_IDS
    return tuple(sorted(TIER_B_SOURCES))
...
def _run_source_acceptance_pipeline(
    ...
    *,
    ...
    outcome = run_tier_b_live_validation(source_id, data_root)
...
def run_source_live_acceptance(source_id: str, *, data_root: Path) -> SourceAcceptanceResult:
    """Run validation_fetch→raw→F0 for one Tier B source."""
...
def build_live_tier_b_evidence_manifest(
    *,
    source_id: str,
    data_root: Path,
    run_id: str,
...
def write_live_tier_b_evidence_manifest(
...
def _process_source_for_report(
    ...
    *,
    ...
    binding = source_bindings()[source_id]
    manifest = build_live_tier_b_evidence_manifest(
        source_id=source_id,
        data_root=data_root,
        run_id=run_id,
        job_id=job_id,
    ...
    manifest_path = write_live_tier_b_evidence_manifest(manifest, data_root=data_root)
    detail = pipeline.detail
...
def write_acceptance_failure_artifact(
    ...
    *,
    ...
    out = Path(report_path).parent / f"tier_b_live_acceptance_failure_{run_id}.json"
...
def run_acceptance_report(
    ...
    *,
    ...
    """Write TierBLiveAcceptanceReport JSON and per-source evidence manifests."""
    try:
        sources = select_source_ids(source_id=source_id, quick=quick)
        resolved_root = validate_live_acceptance_env(data_root=data_root, source_ids=sources)
    except TierBLiveEnvError as exc:
    ...
    report = {
        "command": "tier_b_live_acceptance --report",
        "schema_version": SCHEMA_VERSION,
...
__all__ = [
    ...
    "SCHEMA_VERSION",
    "SOURCE_API_KEY_ENV",
    "TIER_B_SANDBOX_SEGMENT",
    "SourceAcceptanceResult",
    "TierBLiveEnvError",
    "assert_isolated_live_data_root",
    "build_live_tier_b_evidence_manifest",
    ...
    "write_acceptance_failure_artifact",
    "write_live_tier_b_evidence_manifest",
]
