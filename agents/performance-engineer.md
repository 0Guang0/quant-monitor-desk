...
1. **Audit A6：** `AUDIT.plan.md` §1 A6（SKIP → 仅在 §3.6 注 SKIP）
2. **Execute：** MASTER §10 perf + `implement.jsonl`
...
1. 性能要求 = MASTER/AUDIT **冻结**阈值（非自述 KPI）
...
| **HTTP/API 延迟**      | httpx/压测脚本；p95 仅作 §10 冻结指标，非口号                        |
...
- **Audit：** `audit.report.md` §3.6 — 指标 | 阈值 | 实测 | 证据
- **Execute：** `execute-evidence` 含优化前后数字与命令
