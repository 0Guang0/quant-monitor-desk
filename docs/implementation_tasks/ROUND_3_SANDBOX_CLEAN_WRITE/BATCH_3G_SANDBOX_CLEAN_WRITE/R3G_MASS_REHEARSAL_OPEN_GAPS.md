...
| G2  | 日期窗 = 自然日硬编码，非「最新交易日−120交易日」          | ❌   | R3G-01 §4.x TradingCalendar **idea**；`live_pilot_fetch_ports` 有 `recent N trading days` **估算** | **R3H-02 或 Batch3H 运维**（**L2 拷改** EasyXT `smart_data_detector` 日历段） | cap 校验 `_window_days` 也是自然日                                                                                                                                                                                                                                    |
...
| G10 | FRED promote「live」仅验授权，未真拉 API 写库              | ⚠️   | R3E FRED sandbox pilot（**sandbox-only**）                                                         | **R3E + Batch 3H fred 活卡**                                                  | **已接 `--live-wire`**：`fred_live_fetch_evidence.json`（`series[].rows`+`observation_date`）≠ promote `fred_evidence.json`（`observations[].date`）；须 `live_evidence_bridge` 物化 `retrieved_at`/`as_of_timestamp`+DH sidecar；**R3E 链与 promote 链未产品级合一** |
...
| G14 | live 证据缺 DH 门禁 sidecar（closeout/validation_summary） | ⚠️   | 接线暴露 @ 2026-06-27                                                                              | **Batch 3H DH 契约** 或桥接层保留                                             | `live_evidence_bridge._write_sandbox_rehearsal_gate_sidecars` 为 **pilot 专用**；非生产语义                                                                                                                                                                           |
...
| G17 | baostock live promote DH WARNING（日历缺口）               | ⚠️   | `market_bar_p0`                                                                                    | **G2 交易日 SSOT**                                                            | 不阻断 pilot；生产前须闭合                                                                                                                                                                                                                                            |
...
- G1 G2 G3 G4 G5 G6 G10 G11 → **Batch 3H / R3X / 运维切片**，不要回流开 R3G-04 Trellis 全量 Plan。
...
2. **`--live-wire --execute`**：baostock/fred 真网→pilot（G1/G10/G11 部分闭合）
3. Batch 3H：域分表 + OHLCV（G3 G4 G5）+ DH 契约替代 sidecar 补丁（G14）
...
**规划主索引（本文）：** 接线新项 **G14–G17** 与 G1/G10/G11 状态更新**必须**写回本文 §2，已执行 @ 2026-06-27。
