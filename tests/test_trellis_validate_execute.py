...
def _load_validate():
    path = _SCRIPTS / "common" / "validate_execute_handoff.py"
    spec = importlib.util.spec_from_file_location("validate_execute_handoff", path)
    mod = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    sys.modules["validate_execute_handoff"] = mod
    # satisfy relative imports inside common/
...
def _boot_artifacts(task_dir: Path, *, steps: list[str]) -> None:
    ...
    (research / "execute-boot.md").write_text("Phase 0 complete\n", encoding="utf-8")
...
def test_validateExecuteStep_requiresFailSignal(tmp_path: Path) -> None:
    ...
    errors = validate.validate_execute_step(tmp_path, "8.1")
...
def test_validateExecuteStep_passesWithEvidence(tmp_path: Path) -> None:
    ...
    assert validate.validate_execute_step(tmp_path, "8.1") == []


def test_validateExecuteHandoff_failsWithoutBoot(tmp_path: Path) -> None:
    _write_master(tmp_path, ["8.0"])
    errors = validate.validate_execute_handoff(tmp_path, _REPO)
...
def test_parseExecutedSteps_findsMarkedSteps() -> None:
    text = "### 8.0 x\n| 已执行 | [x] |\n\n### 8.1 y\n| 已执行 | [ ] |\n"
    assert validate._parse_executed_steps("## 8.\n" + text) == ["8.0"]
